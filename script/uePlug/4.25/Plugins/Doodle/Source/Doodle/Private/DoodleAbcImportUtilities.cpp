...
//FMatrix DoodleAbcImporterUtilities::ConvertAlembicMatrix(const Alembic::Abc::M44d& AbcMatrix) {
//  FMatrix Matrix;
...
//}
//
//uint32 DoodleAbcImporterUtilities::GenerateMaterialIndicesFromFaceSets(Alembic::AbcGeom::IPolyMeshSchema& Schema, const Alembic::Abc::ISampleSelector FrameSelector, TArray<int32>& MaterialIndicesOut) {
...
//}
//
//void DoodleAbcImporterUtilities::RetrieveFaceSetNames(Alembic::AbcGeom::IPolyMeshSchema& Schema, TArray<FString>& NamesOut) {
//  // Retrieve face set names to determine if we will have to process face sets (used for face-material indices)
//  std::vector<std::string> FaceSetNames;
//  Schema.getFaceSetNames(FaceSetNames);
//
...
//}
//
//void DoodleAbcImporterUtilities::TriangulateIndexBuffer(const TArray<uint32>& InFaceCounts, TArray<uint32>& InOutIndices) {
...
//}
//
//void DoodleAbcImporterUtilities::TriangulateMaterialIndices(const TArray<uint32>& InFaceCounts, TArray<int32>& InOutData) {
//  check(InFaceCounts.Num() > 0);
...
//}
//
//FAbcMeshSample* DoodleAbcImporterUtilities::GenerateAbcMeshSampleForFrame(const Alembic::AbcGeom::IPolyMeshSchema& Schema, const Alembic::Abc::ISampleSelector FrameSelector, const DoESampleReadFlags ReadFlags, const bool bFirstFrame) {
...
//}
//
//DoESampleReadFlags DoodleAbcImporterUtilities::GenerateAbcMeshSampleReadFlags(const Alembic::AbcGeom::IPolyMeshSchema& Schema) {
//  DoESampleReadFlags Flags = DoESampleReadFlags::Default;
//
//  if (Schema.getPositionsProperty().valid() && !Schema.getPositionsProperty().isConstant()) {
//    Flags |= DoESampleReadFlags::Positions;
//  }
//
//  if (Schema.getFaceIndicesProperty().valid() && !Schema.getFaceIndicesProperty().isConstant()) {
//    Flags |= DoESampleReadFlags::Indices;
//  }
//
//  if (Schema.getNormalsParam().valid() && !Schema.getNormalsParam().isConstant()) {
//    Flags |= DoESampleReadFlags::Normals;
//  }
//
...
//
//  if (!bConstantColors) {
//    Flags |= DoESampleReadFlags::Colors;
//  }
//
//  {
...
//      Flags |= DoESampleReadFlags::MaterialIndices;
//    }
//  }
//
//  return Flags;
...
//void DoodleAbcImporterUtilities::GenerateSmoothingGroups(TMultiMap<uint32, uint32>& TouchingFaces, const TArray<FVector>& FaceNormals,
...
//}
//
//bool DoodleAbcImporterUtilities::GenerateAbcMeshSampleDataForFrame(const Alembic::AbcGeom::IPolyMeshSchema& Schema, const Alembic::Abc::ISampleSelector FrameSelector, FAbcMeshSample*& Sample, const DoESampleReadFlags ReadFlags, const bool bFirstFrame) {
...
//  if (EnumHasAnyFlags(ReadFlags, DoESampleReadFlags::Positions)) {
...
//  if (EnumHasAnyFlags(ReadFlags, DoESampleReadFlags::Indices | DoESampleReadFlags::UVs | DoESampleReadFlags::Normals | DoESampleReadFlags::Colors | DoESampleReadFlags::MaterialIndices)) {
...
//  }
//
//  if (EnumHasAnyFlags(ReadFlags, DoESampleReadFlags::Indices)) {
...
//  Alembic::AbcGeom::ICompoundProperty GeomParams = Schema.getArbGeomParams();
//
//  if (EnumHasAnyFlags(ReadFlags, DoESampleReadFlags::UVs)) {
...
//  }
//
//  if (EnumHasAnyFlags(ReadFlags, DoESampleReadFlags::Normals)) {
...
//  }
//
//  if (EnumHasAnyFlags(ReadFlags, DoESampleReadFlags::Colors)) {
//    Alembic::AbcGeom::IC3fGeomParam Color3Property;
//    Alembic::AbcGeom::IC4fGeomParam Color4Property;
//    if (GeomParams.valid()) {
//      const int32 NumGeomParams = GeomParams.getNumProperties();
...
//  }
//
//  if (EnumHasAnyFlags(ReadFlags, DoESampleReadFlags::MaterialIndices)) {
...
//}
//
//void DoodleAbcImporterUtilities::ReadUVSetData(Alembic::AbcGeom::IV2fGeomParam& UVCoordinateParameter, const Alembic::Abc::ISampleSelector FrameSelector, TArray<FVector2D>& OutUVs, const TArray<uint32>& MeshIndices, const bool bNeedsTriangulation, const TArray<uint32>& FaceCounts, const int32 NumVertices) {
...
//}
//
//void DoodleAbcImporterUtilities::GenerateSmoothingGroupsIndices(FAbcMeshSample* MeshSample, float HardEdgeAngleThreshold) {
//  // Vertex lookup map
...
//}
//
//void DoodleAbcImporterUtilities::CalculateNormals(FAbcMeshSample* Sample) {
...
//}
//
//void DoodleAbcImporterUtilities::CalculateSmoothNormals(FAbcMeshSample* Sample) {
...
//}
//
//void DoodleAbcImporterUtilities::CalculateNormalsWithSmoothingGroups(FAbcMeshSample* Sample, const TArray<uint32>& SmoothingMasks, const uint32 NumSmoothingGroups) {
...
//}
//
//void DoodleAbcImporterUtilities::CalculateNormalsWithSampleData(FAbcMeshSample* Sample, const FAbcMeshSample* SourceSample) {
...
//}
//
//void DoodleAbcImporterUtilities::ComputeTangents(FAbcMeshSample* Sample, bool bIgnoreDegenerateTriangles, IMeshUtilities& MeshUtilities) {
...
//}
//
//FAbcMeshSample* DoodleAbcImporterUtilities::MergeMeshSamples(const TArray<const FAbcMeshSample*>& Samples) {
...
//}
//
//FAbcMeshSample* DoodleAbcImporterUtilities::MergeMeshSamples(FAbcMeshSample* MeshSampleOne, FAbcMeshSample* MeshSampleTwo) {
...
//void DoodleAbcImporterUtilities::GetHierarchyForObject(const Alembic::Abc::IObject& Object, TDoubleLinkedList<Alembic::AbcGeom::IXform>& Hierarchy) {
...
//    if (DoodleAbcImporterUtilities::IsType<Alembic::AbcGeom::IXform>(Parent.getMetaData())) {
...
//}
//
//void DoodleAbcImporterUtilities::PropogateMatrixTransformationToSample(FAbcMeshSample* Sample, const FMatrix& Matrix) {
...
//}
//
//void DoodleAbcImporterUtilities::GenerateDeltaFrameDataMatrix(const TArray<FVector>& FrameVertexData, TArray<FVector>& AverageVertexData, const int32 SampleOffset, const int32 AverageVertexOffset, TArray<float>& OutGeneratedMatrix) {
...
//}
//
//void DoodleAbcImporterUtilities::GenerateCompressedMeshData(FCompressedAbcData& CompressedData, const uint32 NumUsedSingularValues, const uint32 NumSamples, const TArray<float>& BasesMatrix, const TArray<float>& BasesWeights, const float SampleTimeStep, const float StartTime) {
...
//}
//
//void DoodleAbcImporterUtilities::CalculateNewStartAndEndFrameIndices(const float FrameStepRatio, int32& InOutStartFrameIndex, int32& InOutEndFrameIndex) {
...
//}
//
//bool DoodleAbcImporterUtilities::AreVerticesEqual(const FSoftSkinVertex& V1, const FSoftSkinVertex& V2) {
...
//}
//
//void DoodleAbcImporterUtilities::ApplyConversion(FAbcMeshSample* InOutSample, const FAbcConversionSettings& InConversionSettings, const bool bShouldInverseBuffers) {
...
//}
//
//bool DoodleAbcImporterUtilities::IsObjectVisible(const Alembic::Abc::IObject& Object, const Alembic::Abc::ISampleSelector FrameSelector) {
...
//}
//
//bool DoodleAbcImporterUtilities::IsObjectVisibilityConstant(const Alembic::Abc::IObject& Object) {
//  checkf(Object.valid(), TEXT("Invalid Object"));
//
//  Alembic::Abc::ICompoundProperty CompoundProperty = Object.getProperties();
//  Alembic::AbcGeom::IVisibilityProperty visibilityProperty;
...
//}
//
//FBoxSphereBounds DoodleAbcImporterUtilities::ExtractBounds(Alembic::Abc::IBox3dProperty InBoxBoundsProperty) {
...
//}
//
//void DoodleAbcImporterUtilities::ApplyConversion(FMatrix& InOutMatrix, const FAbcConversionSettings& InConversionSettings) {
//  // Calculate conversion matrix
...
//}
//
//void DoodleAbcImporterUtilities::ApplyConversion(FBoxSphereBounds& InOutBounds, const FAbcConversionSettings& InConversionSettings) {
//  // Calculate conversion matrix
//  const FMatrix ConversionMatrix = FScaleMatrix::Make(InConversionSettings.Scale) * FRotationMatrix::Make(FQuat::MakeFromEuler(InConversionSettings.Rotation));
...
//}
//
//void DoodleAbcImporterUtilities::ApplyConversion(TArray<FMatrix>& InOutMatrices, const FAbcConversionSettings& InConversionSettings) {
//  // Calculate conversion matrix
//  const FMatrix ConversionMatrix = FScaleMatrix::Make(InConversionSettings.Scale) * FRotationMatrix::Make(FQuat::MakeFromEuler(InConversionSettings.Rotation));
//  if (!ConversionMatrix.Equals(FMatrix::Identity)) {
...
#undef LOCTEXT_NAMESPACE  // "DoodleAbcImporterUtilities"
