...
enum class DoESampleReadFlags : uint8
{
	Default = 0,
	Positions = 1 << 1,
	Indices = 1 << 2,
...
ENUM_CLASS_FLAGS(DoESampleReadFlags);
...
namespace DoodleAbcImporterUtilities
{
	...
	//FAbcMeshSample* GenerateAbcMeshSampleForFrame(const Alembic::AbcGeom::IPolyMeshSchema& Schema, const Alembic::Abc::ISampleSelector FrameSelector, const DoESampleReadFlags ReadFlags, const bool bFirstFrame = false);

	///** Generates a sample read bit mask to reduce unnecessary reads / memory allocations */
	//DoESampleReadFlags GenerateAbcMeshSampleReadFlags(const Alembic::AbcGeom::IPolyMeshSchema& Schema);
	...
	//bool GenerateAbcMeshSampleDataForFrame(const Alembic::AbcGeom::IPolyMeshSchema &Schema, const Alembic::Abc::ISampleSelector FrameSelector, FAbcMeshSample* &Sample, const DoESampleReadFlags ReadFlags, const bool bFirstFrame );
