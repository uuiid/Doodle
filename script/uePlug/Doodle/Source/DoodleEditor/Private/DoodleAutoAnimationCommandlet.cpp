...
UAssetImportTask* UDoodleAutoAnimationCommandlet::CreateGeometryImportTask(const FString& InFbxPath) const
{
	...
	k_abc_stting->SamplingSettings.FrameStart = L_Start.Value - Import_Time_Off; //
                                                                            ...
                                                                            bool bImportAnimations) const
{
	...
	K_FBX_F->ImportUI->AnimSequenceImportData->FrameImportRange.Min = L_Start.Value - Import_Time_Off;
	K_FBX_F->ImportUI->AnimSequenceImportData->FrameImportRange.Max = L_End.Value;
	...
	for (UAssetImportTask* Task : ImportTasks)
	{
		if (TArray<UObject*> ImportedObjs = Task->GetObjects(); !ImportedObjs.IsEmpty())
		{
			...
			if (AnimSeq && TmpSkeletalMesh)
			{
				...
				UMovieSceneSection* AnimSection = L_MovieSceneSkeletalAnim->AddNewAnimationOnRow(L_Start - Import_Time_Off, AnimSeq, -1);
				...
				UMovieSceneSection* AnimSection2 = L_MovieSceneSkeletalAnim2->AddNewAnimationOnRow(L_Start - Import_Time_Off, AnimSeq, -1);
			...
			if (!ImportedObjs.IsEmpty())
			{
				...
				if (ImportedObject->GetClass()->IsChildOf(UGeometryCache::StaticClass()))
				{
					...
					UMovieSceneSection* AnimSection = L_MovieSceneGeoTrack->AddNewAnimation(
						L_Start - Import_Time_Off, L_Actor->GetGeometryCacheComponent());
					AnimSection->SetPreRollFrames(50);
					L_Actor->Modify();
					//---------------------------Clone----------------------------
					...
					UMovieSceneSection* AnimSection2 = L_MovieSceneGeoTrack2->AddNewAnimation(
						L_Start - Import_Time_Off, L_Actor2->GetGeometryCacheComponent());
					AnimSection2->SetPreRollFrames(50);
					L_Actor2->Modify();
					//------------------------
					TheLevelSequence->GetMovieScene()->Modify();
...
void UDoodleAutoRender::Main(float)
{
	FAssetRegistryModule& AssetRegistryModule = FModuleManager::LoadModuleChecked<FAssetRegistryModule>(TEXT("AssetRegistry"));
	if (!AssetRegistryModule.Get().IsLoadingAssets())
	{
		// 去除回调
		FSlateApplication::Get().OnPreTick().Remove(OnPreTickDelegateHandle);
		// 解析参数
		FString L_MoviePipelineConfig{};
		FString L_LevelSequence{};
		// FString L_WorldAssetPath{};
		FParse::Value(FCommandLine::Get(), TEXT("-DoodleMoviePipelineConfig="), L_MoviePipelineConfig);
		FParse::Value(FCommandLine::Get(), TEXT("-DoodleLevelSequence="), L_LevelSequence);
		const FSoftObjectPath L_MoviePipelineConfigPath{L_MoviePipelineConfig};
		const FSoftObjectPath L_LevelSequencePath{L_LevelSequence};

		// 创建执行器
		UMoviePipelinePIEExecutor* L_ProcessExecutor = NewObject<UMoviePipelinePIEExecutor>(GetTransientPackage());
		L_ProcessExecutor->OnExecutorFinished().AddUObject(this, &UDoodleAutoRender::OnEnd);

		// if (ULevelSequence* L_Sequence = Cast<ULevelSequence>(L_LevelSequencePath.TryLoad()); !L_Sequence)
		// {
		...
		NewJob->Sequence = L_LevelSequencePath;
		...
		// 开始执行
		L_MoviePipelineQueueSubsystem->RenderQueueWithExecutorInstance(L_ProcessExecutor);
	}
}

bool UDoodleAutoRender::HasRenderParam()
{
	FString L_MoviePipelineConfig{};
	FString L_LevelSequence{};
	// FString L_WorldAssetPath{};
	FParse::Value(FCommandLine::Get(), TEXT("-DoodleMoviePipelineConfig="), L_MoviePipelineConfig);
	FParse::Value(FCommandLine::Get(), TEXT("-DoodleLevelSequence="), L_LevelSequence);
	// FParse::Value(FCommandLine::Get(), TEXT("DoodleWorldAssetPath"), L_WorldAssetPath);
	return L_MoviePipelineConfig.Len() > 0 && L_LevelSequence.Len() > 0;
}

void UDoodleAutoRender::OnEnd(UMoviePipelineExecutorBase*, bool)
{
