...
struct FCreateCharacterSliderController::FScrubRangeToScreen {
  double ViewStart;
  ...
  float InputToLocalX(double Input) const { return (Input - ViewStart) * PixelsPerInput; }
};
...
FFrameTime FCreateCharacterSliderController::ComputeFrameTimeFromMouse(
    const FGeometry& Geometry, FVector2D ScreenSpacePosition, FScrubRangeToScreen RangeToScreen, bool CheckSnapping
) const {
...
FCreateCharacterSliderController::FScrubPixelRange FCreateCharacterSliderController::GetHitTestScrubberPixelRange(
    FFrameTime ScrubTime, const FScrubRangeToScreen& RangeToScreen
) const {
  static const float DragToleranceSlateUnits = 2.f, MouseTolerance = 2.f;
  return GetScrubberPixelRange(
      ScrubTime, GetTickResolution(), GetDisplayRate(), RangeToScreen, DragToleranceSlateUnits + MouseTolerance
  );
}

FCreateCharacterSliderController::FScrubPixelRange FCreateCharacterSliderController::GetScrubberPixelRange(
    FFrameTime ScrubTime, const FScrubRangeToScreen& RangeToScreen
) const {
...
FCreateCharacterSliderController::FScrubPixelRange FCreateCharacterSliderController::GetScrubberPixelRange(
  ...
  const FFrameNumber Frame =
      ScrubTime.FloorToFrame();  // ConvertFrameTime(ScrubTime, Resolution, PlayRate).FloorToFrame();
  ...
  if (Range.bClamped) {
    Range.HandleRange =
        TRange<float>((StartPixel + EndPixel - MinScrubSize) * .5f, (StartPixel + EndPixel + MinScrubSize) * .5f);
  } else {
...
struct FCreateCharacterSliderController::FDrawTickArgs {
...
void FCreateCharacterSliderController::DrawTicks(
    FSlateWindowElementList& OutDrawElements, const TRange<double>& ViewRange, const FScrubRangeToScreen& RangeToScreen,
  ...
  for (double CurrentMajorLine = FirstMajorLine; CurrentMajorLine < LastMajorLine; CurrentMajorLine += MajorGridStep) {
    ...
    FSlateDrawElement::MakeLines(
        OutDrawElements, InArgs.StartLayer, PaintGeometry, LinePoints, InArgs.DrawEffects, InArgs.TickColor,
        bAntiAliasLines
    );

    if (!InArgs.bOnlyDrawMajorTicks) {
      FString FrameString =
          TimeSliderArgs.NumericTypeInterface->ToString((CurrentMajorLine * FrameResolution).RoundToFrame().Value);

      // Space the text between the tick mark but slightly above
      FVector2D TextOffset(
          MajorLinePx + 5.f,
          InArgs.bMirrorLabels ? 3.f : FMath::Abs(InArgs.AllottedGeometry.Size.Y - (InArgs.MajorTickHeight + 3.f))
      );
...
int32 FCreateCharacterSliderController::OnPaintTimeSlider(
  ...
  if (LocalSequenceLength > 0) {
    ...
    LayerId =
        DrawPlaybackRange(AllottedGeometry, MyCullingRect, OutDrawElements, LayerId, RangeToScreen, PlaybackRangeArgs);

    PlaybackRangeArgs.SolidFillOpacity = 0.05f;
    LayerId =
        DrawSelectionRange(AllottedGeometry, MyCullingRect, OutDrawElements, LayerId, RangeToScreen, PlaybackRangeArgs);
    ...
    const float HandleStart =
        RangeToScreen.InputToLocalX(TimeSliderArgs.ScrubPosition.Get().AsDecimal() / GetTickResolution().AsDecimal()) -
        7.0f;
    ...
    FSlateDrawElement::MakeBox(OutDrawElements, ArrowLayer, MyGeometry, Brush, DrawEffects, ScrubColor);

    {
      ...
      if (GetDefault<UPersonaOptions>()->bTimelineDisplayPercentage) {
        double Percentage = FMath::Clamp(
            TimeSliderArgs.ScrubPosition.Get().AsDecimal() /
                FFrameTime(LocalPlaybackRange.Size<FFrameNumber>()).AsDecimal(),
            0.0, 1.0
        );
      ...
      FVector2D TextOffset(
          TextPosition,
          Args.bMirrorLabels ? TextSize.Y - 6.f : Args.AllottedGeometry.Size.Y - (Args.MajorTickHeight + TextSize.Y)
      );
...
int32 FCreateCharacterSliderController::DrawSelectionRange(
...
int32 FCreateCharacterSliderController::DrawPlaybackRange(
    const FGeometry& AllottedGeometry, const FSlateRect& MyCullingRect, FSlateWindowElementList& OutDrawElements,
    int32 LayerId, const FScrubRangeToScreen& RangeToScreen, const FPaintPlaybackRangeArgs& Args
) const {
...
int32 FCreateCharacterSliderController::DrawEditableTimes(
    const FGeometry& AllottedGeometry, const FSlateRect& MyCullingRect, FSlateWindowElementList& OutDrawElements,
  ...
  //      OutDrawElements,
  //      LayerId + 1,
  //      AllottedGeometry.ToPaintGeometry(FVector2D(LinePos - 6.0f, AllottedGeometry.Size.Y - 12.0f),
  //      FVector2D(11.0f, 12.0f)), EditableTimeBrush, ESlateDrawEffect::None, TimeColor
  //  );
  //}
...
FReply FCreateCharacterSliderController::OnMouseButtonDown(
    SWidget& WidgetOwner, const FGeometry& MyGeometry, const FPointerEvent& MouseEvent
) {
  DistanceDragged      = 0;
...
FReply FCreateCharacterSliderController::OnMouseButtonUp(
    SWidget& WidgetOwner, const FGeometry& MyGeometry, const FPointerEvent& MouseEvent
) {
  bool bHandleLeftMouseButton =
      MouseEvent.GetEffectingButton() == EKeys::LeftMouseButton && WidgetOwner.HasMouseCapture();
  bool bHandleRightMouseButton = MouseEvent.GetEffectingButton() == EKeys::RightMouseButton &&
    ...
    if (MouseDragType == DRAG_PLAYBACK_START) {
      ...
      if (bCanZoomIn || bCanZoomOut) {
        ...
        if (bCanZoomIn) {
          ...
          ViewRange = TRange<double>(
              MouseDownStart.FrameNumber / GetTickResolution(), MouseTime.FrameNumber / GetTickResolution()
          );
        }
...
FReply FCreateCharacterSliderController::OnMouseMove(
    SWidget& WidgetOwner, const FGeometry& MyGeometry, const FPointerEvent& MouseEvent
) {
  ...
  if (bHandleRightMouseButton) {
    ...
    if (MouseDragType == DRAG_NONE) {
      if (DistanceDragged > FSlateApplication::Get().GetDragTriggerDistance()) {
        ...
        if (!SelectionRange.IsEmpty() && !bHitScrubber &&
            HitTestRangeEnd(RangeToScreen, SelectionRange, MouseDownPixel)) {
...
void FCreateCharacterSliderController::CommitScrubPosition(FFrameTime NewValue, bool bIsScrubbing) {
...
FReply FCreateCharacterSliderController::OnMouseWheel(
  ...
  if (TimeSliderArgs.AllowZoom && MouseEvent.IsControlDown()) {
    float MouseFractionX =
        MyGeometry.AbsoluteToLocal(MouseEvent.GetScreenSpacePosition()).X / MyGeometry.GetLocalSize().X;
...
FCursorReply FCreateCharacterSliderController::OnCursorQuery(
  ...
  const bool bHitScrubber          = GetHitTestScrubberPixelRange(TimeSliderArgs.ScrubPosition.Get(), RangeToScreen)
                                .HandleRange.Contains(HitTestPixel);
  ...
  if (bHitTime || (MouseDragType == DRAG_PLAYBACK_END) || (MouseDragType == DRAG_PLAYBACK_START) ||
      (MouseDragType == DRAG_SELECTION_START) || (MouseDragType == DRAG_SELECTION_END) ||
      (MouseDragType == DRAG_TIME) ||
...
int32 FCreateCharacterSliderController::OnPaintViewArea(
    const FGeometry& AllottedGeometry, const FSlateRect& MyCullingRect, FSlateWindowElementList& OutDrawElements,
...
FFrameTime FCreateCharacterSliderController::GetFrameTimeFromMouse(
    const FGeometry& Geometry, FVector2D ScreenSpacePosition
) const {
...
TSharedRef<SWidget> FCreateCharacterSliderController::OpenSetPlaybackRangeMenu(FFrameNumber FrameNumber) {
...
void FCreateCharacterSliderController::ClampViewRange(double& NewRangeMin, double& NewRangeMax) {
...
void FCreateCharacterSliderController::SetViewRange(
    double NewRangeMin, double NewRangeMax, EViewRangeInterpolation Interpolation
) {
...
void FCreateCharacterSliderController::SetClampRange(double NewRangeMin, double NewRangeMax) {
...
void FCreateCharacterSliderController::SetPlayRange(FFrameNumber RangeStart, int32 RangeDuration) {
...
bool FCreateCharacterSliderController::ZoomByDelta(float InDelta, float MousePositionFraction) {
...
void FCreateCharacterSliderController::PanByDelta(float InDelta) {
  TRange<double> LocalViewRange = TimeSliderArgs.ViewRange.Get().GetAnimationTarget();
...
bool FCreateCharacterSliderController::HitTestRangeStart(
    const FScrubRangeToScreen& RangeToScreen, const TRange<double>& Range, float HitPixel
) const {
...
bool FCreateCharacterSliderController::HitTestRangeEnd(
    const FScrubRangeToScreen& RangeToScreen, const TRange<double>& Range, float HitPixel
) const {
...
bool FCreateCharacterSliderController::HitTestTime(
    const FScrubRangeToScreen& RangeToScreen, double Time, float HitPixel
) const {
...
void FCreateCharacterSliderController::SetPlaybackRangeStart(FFrameNumber NewStart) {
  TRange<FFrameNumber> PlaybackRange = TimeSliderArgs.PlaybackRange.Get();
...
void FCreateCharacterSliderController::SetPlaybackRangeEnd(FFrameNumber NewEnd) {
  TRange<FFrameNumber> PlaybackRange = TimeSliderArgs.PlaybackRange.Get();
...
void FCreateCharacterSliderController::SetSelectionRangeStart(FFrameNumber NewStart) {
  TRange<FFrameNumber> SelectionRange = TimeSliderArgs.SelectionRange.Get();
...
void FCreateCharacterSliderController::SetSelectionRangeEnd(FFrameNumber NewEnd) {
  TRange<FFrameNumber> SelectionRange = TimeSliderArgs.SelectionRange.Get();
...
int32 FCreateCharacterSliderController::HitTestTimes(const FScrubRangeToScreen& RangeToScreen, float HitPixel) const {
...
void FCreateCharacterSliderController::SetEditableTime(int32 TimeIndex, float Time, bool bIsDragging) {
