...
{
public:
	FCreateCharacterCurveEditorBounds(SCreateCharacterCurveEditor* In_Edit)
		: Editor(In_Edit)
	{
	}
	...
	) override
	{
		if (InColumnName == ColumnNames.Label)
		{
			return SNew(SHorizontalBox)
				...
				.AutoWidth()
				[
					SNew(STextBlock).Text(CurveName).ColorAndOpacity(FSlateColor(Curve_Color))
				];
		}
...
void SCreateCharacterCurveEditor::Construct(const FArguments& InArgs)
{
	...
	FTimeSliderArgs TimeSliderArgs;
	{
		...
		TimeSliderArgs.OnClampRangeChanged =
			FOnTimeRangeChanged::CreateLambda([this](TRange<double> In_Range)
			{
