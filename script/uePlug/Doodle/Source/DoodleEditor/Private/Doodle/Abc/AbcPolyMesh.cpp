...
static const ESampleReadFlags ReadAllFlags =
    ESampleReadFlags::Positions | ESampleReadFlags::Indices | ESampleReadFlags::UVs | ESampleReadFlags::Normals |
    ESampleReadFlags::Colors | ESampleReadFlags::MaterialIndices | ESampleReadFlags::Velocities;

FAbcPolyMesh::FAbcPolyMesh(
    const Alembic::AbcGeom::IPolyMesh& InPolyMesh, const FAbcFile* InFile, IAbcObject* InParent /*= nullptr*/
)
    : IAbcObject(InPolyMesh, InFile, InParent),
      SelfBounds(EForceInit::ForceInitToZero),
      ChildBounds(EForceInit::ForceInitToZero),
      bShouldImport(true),
      PolyMesh(InPolyMesh),
      Schema(InPolyMesh.getSchema()),
      FirstSample(nullptr),
      TransformedFirstSample(nullptr),
      SampleReadFlags(ReadAllFlags),
      bReturnFirstSample(false),
      bReturnTransformedFirstSample(false),
      bFirstFrameVisibility(true) {
  ...
  /** Yuck, but retrieving face set is actually a file read operation as they are not cached on the schema (only
   * function that is non-const) */
...
bool FAbcPolyMesh::ReadFirstFrame(const float InTime, const int32 FrameIndex) {
  ...
  FirstSample =
      AbcImporterUtilities::GenerateAbcMeshSampleForFrame(Schema, SampleSelector, SampleReadFlags, bFirstFrame);
  ...
  if (FirstSample) {
    if (FirstSample->Normals.Num() == 0) {
      // Normals are not available so they should be calculated each frame, so force set the Normals read flag which
      // will ensure they are generated
    ...
    AbcImporterUtilities::ComputeTangents(
        FirstSample, File->GetImportSettings()->NormalGenerationSettings.bIgnoreDegenerateTriangles,
    ...
    if (bConstant && bConstantTransformation && !bApplyTransformation) {
      ...
      // Resident samples from initial sample (this copies all vertex attributes, which allows us to skip re-reading
      // constant attributes in future sample reads)
      for (int32 Index = 0; Index < MaxNumberOfResidentSamples; ++Index) {
...
void FAbcPolyMesh::CalculateNormalsForSample(FAbcMeshSample* Sample) {
  ...
  */
  ...
  if (File->GetImportSettings()->NormalGenerationSettings.bForceOneSmoothingGroupPerObject && bRecomputeNormals) {
    ...
    // Otherwise we'd expect normals to be available, if not we should assume the object has smooth normals (so
    // calculate them)
    ...
    if (bNormalsAvailable || bRecomputeNormals) {
      ...
      AbcImporterUtilities::GenerateSmoothingGroupsIndices(
          Sample, File->GetImportSettings()->NormalGenerationSettings.HardEdgeAngleThreshold
      );
    }

    // In case we are expected to recompute the normals now recalculate the normals using the calculated smoothing
    // groups
    if (bRecomputeNormals) {
      AbcImporterUtilities::CalculateNormalsWithSmoothingGroups(
          Sample, Sample->SmoothingGroupIndices, Sample->NumSmoothingGroups
      );
    }
  }
}

void FAbcPolyMesh::SetFrameAndTime(
    const float InTime, const int32 FrameIndex, const EFrameReadFlags InFlags, const int32 TargetIndex /*= INDEX_NONE*/
) {
  if (!bShouldImport) {
    return;
  }
  ...
  if (TargetIndex != INDEX_NONE) {
    ...
    if (!bConstant) {
      const bool bRecomputeNormals = File->GetImportSettings()->NormalGenerationSettings.bRecomputeNormals;
      const ESampleReadFlags SampleReadFlagsOverride = EnumHasAnyFlags(InFlags, EFrameReadFlags::PositionAndNormalOnly)
                                                           ? ESampleReadFlags::Positions | ESampleReadFlags::Normals
                                                           : SampleReadFlags;
      WriteSample->Copy(FirstSample, SampleReadFlagsOverride);
      const bool bValidSample = AbcImporterUtilities::GenerateAbcMeshSampleDataForFrame(
          Schema, SampleSelector, WriteSample, SampleReadFlagsOverride, InTime == MinTime
      );
      ...
      // Make sure in case of recomputing normals we enforece using the first sample data (otherwise we'll be using
      // loaded or incorrectly calculated normals)
      if (WriteSample->Normals.Num() == 0 || bRecomputeNormals) {
        // If the indices match we can recalculate the normals according to the first frame (and copy the smoothing
        // indices)
        if (bMatchingIndices) {
        ...
        if (!File->GetImportSettings()->NormalGenerationSettings.bForceOneSmoothingGroupPerObject) {
          AbcImporterUtilities::GenerateSmoothingGroupsIndices(
              WriteSample, File->GetImportSettings()->NormalGenerationSettings.HardEdgeAngleThreshold
          );
        } else {
...
bool FAbcPolyMesh::HasConstantTransform() const { return bConstantTransformation; }
...
const FAbcMeshSample* FAbcPolyMesh::GetFirstSample() const { return FirstSample; }

const FAbcMeshSample* FAbcPolyMesh::GetTransformedFirstSample() const { return TransformedFirstSample; }

ESampleReadFlags FAbcPolyMesh::GetSampleReadFlags() const { return SampleReadFlags; }
