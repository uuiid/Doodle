...
class FAbcFile {
 public:
  ...
  EAbcImportError Import(UDoodleAbcImportSettings* InImportSettings);
  ...
  /** Reads the frame data for the given FrameIndex, with the ReadIndex used to specify the current read when doing
   * concurrent reads (up to 8 at the same time) */
  void ReadFrame(int32 FrameIndex, const EFrameReadFlags InFlags, const int32 ReadIndex = INDEX_NONE);
  /** Cleans up frame data. Must be called after ReadFrame when the frame data (with matching ReadIndex) is not needed
   * anymore */
