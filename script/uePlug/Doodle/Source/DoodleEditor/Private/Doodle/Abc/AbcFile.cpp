...
#include "Doodle/Abc/AbcPolyMesh.h"
#include "Doodle/Abc/AbcTransform.h"
#include "HAL/Event.h"
...
EAbcImportError FAbcFile::Open() {
  ...
  if (!Archive.valid()) {
    ...
    if (CompressionType == Alembic::AbcCoreFactory::IFactory::CoreType::kHDF5) {
      Message = FTokenizedMessage::Create(
          EMessageSeverity::Error,
          LOCTEXT(
              "AbcHDF5NotSupported",
              "HDF5 Alembic files are not supported. Please convert the file to Ogawa format before importing."
          )
      );
    } else {
      Message = FTokenizedMessage::Create(
          EMessageSeverity::Error, LOCTEXT("AbcUnknownFormat", "Unknown file format: not a valid Alembic.")
      );
    }
  ...
  if (!TopObject.valid()) {
    TSharedRef<FTokenizedMessage> Message = FTokenizedMessage::Create(
        EMessageSeverity::Error, LOCTEXT("AbcInvalidRoot", "Invalid root node: cannot proceed with import.")
    );
  ...
  Alembic::Abc::IBox3dProperty ArchiveBoundsProperty =
      Alembic::AbcGeom::GetIArchiveBounds(Archive, Alembic::Abc::ErrorHandler::kQuietNoopPolicy);
...
TArray<FAbcFile::FMetaData> FAbcFile::GetArchiveMetaData() const {
  ...
  TArray<FMetaData> MetaData(
      {FMetaDataEntry(TEXT("Abc.AppName"), AppName), FMetaDataEntry(TEXT("Abc.LibraryVersion"), LibVersionString),
       FMetaDataEntry(TEXT("Abc.WrittenOn"), DateWritten), FMetaDataEntry(TEXT("Abc.UserDescription"), UserDescription),
...
EAbcImportError FAbcFile::Import(UDoodleAbcImportSettings* InImportSettings) {
  ImportSettings        = InImportSettings;

  // Compute start/end frames based on the settings and report back the computed values to the settings for display and
  // serialization
  ...
  EndFrameIndex         = FMath::Clamp(MaxFrameIndex, LowerFrameIndex, UpperFrameIndex);
  ...
  if (FrameSpan <= 0) {
    TSharedRef<FTokenizedMessage> Message = FTokenizedMessage::Create(
        EMessageSeverity::Error,
        FText::Format(
            LOCTEXT("NoFramesForMeshObject", "Invalid frame range specified {0} - {1}."),
            FText::FromString(FString::FromInt(StartFrameIndex)), FText::FromString(FString::FromInt(EndFrameIndex))
        )
    );
    FAbcImportLogger::AddImportMessage(Message);
  ...
  TimeStep               = !FMath::IsNearlyZero(ArchiveSecondsPerFrame) ? ArchiveSecondsPerFrame
                                                                        : CacheLength / (float)(MaxFrameIndex - MinFrameIndex);
  // switch (SamplingType) {
  ...
  // If there were not bounds available at the archive level or the mesh bounds are larger than the archive bounds use
  // those
  ...
  // Populate the list of unique face set names from the meshes that should be imported regardless of the import
  // material settings
...
void FAbcFile::TraverseAbcHierarchy(const Alembic::Abc::IObject& InObject, IAbcObject* InParent) {
  ...
  if (AbcImporterUtilities::IsType<Alembic::AbcGeom::IPolyMesh>(ObjectMetaData)) {
    ...
    if (Transform->GetNumberOfSamples() > 1) {
      ...
      MaxFrameIndex =
          FMath::Max(MaxFrameIndex, Transform->GetFrameIndexForFirstData() + Transform->GetNumberOfSamples());
    }
  }
...
void FAbcFile::ProcessFrames(TFunctionRef<void(int32, FAbcFile*)> InCallback, const EFrameReadFlags InFlags) {
  const int32 NumWorkerThreads =
      FMath::Clamp(FTaskGraphInterface::Get().GetNumWorkerThreads(), 1, MaxNumberOfResidentSamples);
  const bool bSingleThreaded = ImportSettings->NumThreads == 1 ||
                               EnumHasAnyFlags(InFlags, EFrameReadFlags::ForceSingleThreaded) ||
  ...
  if (bSingleThreaded) {
    ...
    if (!FrameWrittenEvent) {
      TSharedRef<FTokenizedMessage> Message = FTokenizedMessage::Create(
          EMessageSeverity::Error,
          LOCTEXT("NoSynchEvent", "Unable to get synchronization event for parallelized Alembic frame data import.")
      );
      FAbcImportLogger::AddImportMessage(Message);
      return;
    }
    ...
    ParallelFor(
        NumWorkerThreads,
        [this, InFlags, InCallback, bSingleThreaded, NumWorkerThreads, &WriteFrameIndex, &Mutex,
         &FrameWrittenEvent](int32 ThreadIndex) {
...
const int32 FAbcFile::GetMinFrameIndex() const { return MinFrameIndex; }

const int32 FAbcFile::GetMaxFrameIndex() const { return MaxFrameIndex; }

const int32 FAbcFile::GetStartFrameIndex() const { return StartFrameIndex; }
...
const TArray<FAbcTransform*>& FAbcFile::GetTransforms() const { return Transforms; }

const int32 FAbcFile::GetNumPolyMeshes() const { return PolyMeshes.Num(); }

const FString FAbcFile::GetFilePath() const { return FilePath; }

const float FAbcFile::GetImportTimeOffset() const { return ImportTimeOffset; }

const float FAbcFile::GetImportLength() const { return ImportLength; }

const int32 FAbcFile::GetImportNumFrames() const { return EndFrameIndex - StartFrameIndex; }

const int32 FAbcFile::GetFramerate() const { return FramesPerSecond; }

const float FAbcFile::GetSecondsPerFrame() const { return SecondsPerFrame; }
...
const FBoxSphereBounds& FAbcFile::GetArchiveBounds() const { return ArchiveBounds; }
...
IMeshUtilities* FAbcFile::GetMeshUtilities() const { return MeshUtilities; }
