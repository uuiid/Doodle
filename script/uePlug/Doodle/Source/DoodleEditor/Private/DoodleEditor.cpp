...
void FdoodleEditorModule::StartupModule()
{
	...
	FGlobalTabmanager::Get() //文件管理
		...
		.SetMenuType(ETabSpawnerMenuType::Hidden);
	//-----------
	FGlobalTabmanager::Get()->RegisterNomadTabSpawner(UDoodleEffectLibraryWidget::Name,
	                                                  FOnSpawnTab::CreateStatic(&UDoodleEffectLibraryWidget::OnSpawnAction))
	                        .SetDisplayName(FText::FromString(TEXT("特效资源库")))
	                        .SetMenuType(ETabSpawnerMenuType::Hidden);
	//-------------
	FGlobalTabmanager::Get()->RegisterNomadTabSpawner(UDoodleEffectLibraryEditWidget::Name,
	                                                  FOnSpawnTab::CreateStatic(&UDoodleEffectLibraryEditWidget::OnSpawnAction))
	...
	L_Module.RegisterCustomClassLayout(ADoodleCreateCharacterActor::StaticClass()->GetFName(),
	                                   FOnGetDetailCustomizationInstance::CreateStatic(&CreateCharacterActor_Customization::MakeInstance));
	...
	EAssetTypeCategories::Type AssetCategory = L_AssetTools.RegisterAdvancedAssetCategory(
		FName{TEXT("Create Variant")}, LOCTEXT("Doodle", "Doodle Variant"));
	...
	if (UDoodleAutoRender::HasRenderParam())
	{
		UDoodleAutoRender* L_AutoRender = NewObject<UDoodleAutoRender>(GetTransientPackage());
		L_AutoRender->AddToRoot();
		L_AutoRender->OnPreTickDelegateHandle = FSlateApplication::Get().OnPreTick().AddUObject(L_AutoRender, &UDoodleAutoRender::Main);
	}
}
