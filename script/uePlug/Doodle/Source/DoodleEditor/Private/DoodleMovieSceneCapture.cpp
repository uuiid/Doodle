...
#include UE_INLINE_GENERATED_CPP_BY_NAME(DoodleMovieSceneCapture)

#define LOCTEXT_NAMESPACE "DoodleMovieSceneCapture"

const FName UDoodleMovieSceneCapture::MovieSceneCaptureUIName = FName(TEXT("MovieSceneCaptureUIInstance"));
//------------------------------------
...
void FActiveMovieSceneCaptures::Add(UDoodleMovieSceneCapture* Capture)
{
	ActiveCaptures.AddUnique(Capture);
}

void FActiveMovieSceneCaptures::Remove(UDoodleMovieSceneCapture* Capture)
{
...
{
	TArray<UDoodleMovieSceneCapture*> Captures = ActiveCaptures;
	for (UDoodleMovieSceneCapture* Capture : Captures)
	{
...
{
	TArray<UDoodleMovieSceneCapture*> ActiveCapturesCopy;
...
UDoodleMovieSceneCapture::UDoodleMovieSceneCapture(const FObjectInitializer& Initializer)
	: Super(Initializer)
{
...
void UDoodleMovieSceneCapture::PostInitProperties()
{
...
void UDoodleMovieSceneCapture::SetImageCaptureProtocolType(TSubclassOf<UMovieSceneCaptureProtocolBase> ProtocolType)
{
	ImageCaptureProtocolType = ProtocolType.Get();
	InitializeCaptureProtocols();
}

void UDoodleMovieSceneCapture::SetAudioCaptureProtocolType(TSubclassOf<UMovieSceneCaptureProtocolBase> ProtocolType)
{
	AudioCaptureProtocolType = ProtocolType.Get();
	InitializeCaptureProtocols();
}

void UDoodleMovieSceneCapture::ForciblyReinitializeCaptureProtocols()
{
...
#if WITH_EDITOR
		FPropertyChangedEvent PropertyChangedEvent(GetClass()->FindPropertyByName(GET_MEMBER_NAME_CHECKED(UDoodleMovieSceneCapture, ImageCaptureProtocol)), EPropertyChangeType::ValueSet);
...
#if WITH_EDITOR
		FPropertyChangedEvent PropertyChangedEvent(GetClass()->FindPropertyByName(GET_MEMBER_NAME_CHECKED(UDoodleMovieSceneCapture, AudioCaptureProtocol)), EPropertyChangeType::ValueSet);
		PostEditChangeProperty(PropertyChangedEvent);
#endif
	}
...
void UDoodleMovieSceneCapture::InitializeCaptureProtocols()
{
...
void UDoodleMovieSceneCapture::Initialize(TSharedPtr<FSceneViewport> InSceneViewport, int32 PIEInstance)
{
	ensure(!bCapturing);
	...
	{
		CaptureStrategy = MakeShareable(new FRealTimeCaptureStrategy1(Settings.FrameRate));
...
void UDoodleMovieSceneCapture::StartWarmup()
{
	bCapturing = false;
...
void UDoodleMovieSceneCapture::StartCapture()
{
	bFinalizeWhenReady = false;
	bCapturing = true;
...
void UDoodleMovieSceneCapture::CaptureThisFrame(float DeltaSeconds)
{
...
void UDoodleMovieSceneCapture::Tick(float DeltaSeconds)
{
	if (ImageCaptureProtocol)
	{
...
void UDoodleMovieSceneCapture::FinalizeWhenReady()
{
...
void UDoodleMovieSceneCapture::Finalize()
{
	if (Settings.bCinematicEngineScalability)
	{
...
FString UDoodleMovieSceneCapture::ResolveFileFormat(const FString& Format, const FFrameMetrics& FrameMetrics) const
{
...
void UDoodleMovieSceneCapture::LoadFromConfig()
{
	LoadConfig();
	InitializeCaptureProtocols();
	if (ImageCaptureProtocol)
...
void UDoodleMovieSceneCapture::SaveToConfig()
{
...
void UDoodleMovieSceneCapture::SerializeJson(FJsonObject& Object)
{
	if (ImageCaptureProtocol)
	{
...
void UDoodleMovieSceneCapture::DeserializeJson(const FJsonObject& Object)
{
...
bool UDoodleMovieSceneCapture::ShouldFinalize() const
{
	return bFinalizeWhenReady && ImageCaptureProtocol->HasFinishedProcessing() && AudioCaptureProtocol->HasFinishedProcessing();
}

bool UDoodleMovieSceneCapture::IsAudioPassIfNeeded() const
{
	return AudioCaptureProtocolType == UNullAudioCaptureProtocol::StaticClass() || bIsAudioCapturePass;
}

#if WITH_EDITOR
void UDoodleMovieSceneCapture::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
	FName PropertyName = (PropertyChangedEvent.MemberProperty != NULL) ? PropertyChangedEvent.MemberProperty->GetFName() : NAME_None;

	if (PropertyName == GET_MEMBER_NAME_CHECKED(UDoodleMovieSceneCapture, ImageCaptureProtocolType)
		|| PropertyName == GET_MEMBER_NAME_CHECKED(UDoodleMovieSceneCapture, AudioCaptureProtocolType))
	{
		InitializeCaptureProtocols();
	}
...
FFixedTimeStepCaptureStrategy1::FFixedTimeStepCaptureStrategy1(FFrameRate InFrameRate)
	: FrameRate(InFrameRate)
{
}

void FFixedTimeStepCaptureStrategy1::OnInitialize()
{
	FApp::SetFixedDeltaTime(FrameRate.AsInterval());
	FApp::SetUseFixedTimeStep(true);
}

void FFixedTimeStepCaptureStrategy1::OnStop()
{
...
bool FFixedTimeStepCaptureStrategy1::ShouldPresent(double CurrentTimeSeconds, uint32 FrameIndex) const
{
	return true;
}

int32 FFixedTimeStepCaptureStrategy1::GetDroppedFrames(double CurrentTimeSeconds, uint32 FrameIndex) const
{
	return 0;
}

FRealTimeCaptureStrategy1::FRealTimeCaptureStrategy1(FFrameRate InFrameRate)
	: NextPresentTimeS(0), FrameLength(InFrameRate.AsInterval())
{
}

void FRealTimeCaptureStrategy1::OnInitialize()
{
}

void FRealTimeCaptureStrategy1::OnStop()
{
}

bool FRealTimeCaptureStrategy1::ShouldPresent(double CurrentTimeSeconds, uint32 FrameIndex) const
{
	return CurrentTimeSeconds >= FrameIndex * FrameLength;
}

int32 FRealTimeCaptureStrategy1::GetDroppedFrames(double CurrentTimeSeconds, uint32 FrameIndex) const
{
