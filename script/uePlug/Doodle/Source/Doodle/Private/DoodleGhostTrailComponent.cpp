...
void UDoodleGhostTrailComponent::CreateGhost(FVector InLocation,
                                             float DeltaTime)
{
    float LLen =
        FMath::Abs(
...
void UDoodleGhostTrailComponent::UpdataMaterial_Doodle(UPoseableMeshComponent *InGhost,
                                                       float InValue)
{
...
void UDoodleGhostTrailComponent::ClearGhost()
{
    ...
    GhostInfos.RemoveAll([](const FDoodleGhostTrailInfo &In) -> bool
                         { return In.Age > In.Life; });
