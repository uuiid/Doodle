    ...
    FPropertyChangedEvent &PropertyChangeEvent)
{
    ...
    else if (name == GET_MEMBER_NAME_CHECKED(ThisClass, p_vector) ||
             name == GET_MEMBER_NAME_CHECKED(ThisClass, vectorMax) ||
             name == GET_MEMBER_NAME_CHECKED(ThisClass, vectorMin))
    {
        SetVector();
    }
...
void ADoodleSurroundMeshActor::SetSurroundMesh()
{
    ...
    {
        auto l_tran = p_spline->GetTransformAtDistanceAlongSpline((float)i * k_segments,
...
void ADoodleSurroundMeshActor::Tick(float DeltaTime)
{
    ...
    for (int i = 0; i < p_count; ++i)
    {
        auto l_tran = p_spline->GetTransformAtDistanceAlongSpline(p_lens[i],
                                                                  ESplineCoordinateSpace::Type::Local,
                                                                  false);
        ...
        l_tran.AddToTranslation(FVector{noise_x,
                                        noise_y,
                                        noise_z});
