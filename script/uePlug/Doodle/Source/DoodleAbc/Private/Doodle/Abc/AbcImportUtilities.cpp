...
uint32 AbcImporterUtilities::GenerateMaterialIndicesFromFaceSets(
    Alembic::AbcGeom::IPolyMeshSchema& Schema, const Alembic::Abc::ISampleSelector FrameSelector,
    TArray<int32>& MaterialIndicesOut
) {
...
FAbcMeshSample* AbcImporterUtilities::GenerateAbcMeshSampleForFrame(
    const Alembic::AbcGeom::IPolyMeshSchema& Schema, const Alembic::Abc::ISampleSelector FrameSelector,
    const ESampleReadFlags ReadFlags, const bool bFirstFrame
) {
...
void AbcImporterUtilities::GenerateSmoothingGroups(
  ...
  // MultiMap holding connected face indices of which is determined they belong to the same smoothing group (angle
  // between face normals tested)
  ...
  for (int32 FaceIndex = 0; FaceIndex < NumFaces; ++FaceIndex) {
    ...
    for (int32 i = 0; i < ConnectedFaceIndices.Num(); ++i) {
      ...
      if (DotProduct > HardAngleDotThreshold || (bZeroThreshold && FMath::IsNearlyZero(DotProduct))) {
        // If the faces have a "similar" normal we can determine that they should belong to the same smoothing group so
        // mark them as SmoothingGroupConnectedFaces
  ...
  while (ProcessedFaces != NumFaces && CurrentFaceIndex < NumFaces) {
    ...
    if (ConnectedFaceIndices.Num()) {
      ...
      for (int32 FoundConnectedFaceIndex = 0; FoundConnectedFaceIndex < ConnectedFaceIndices.Num();
           ++FoundConnectedFaceIndex) {
...
bool AbcImporterUtilities::GenerateAbcMeshSampleDataForFrame(
    const Alembic::AbcGeom::IPolyMeshSchema& Schema, const Alembic::Abc::ISampleSelector FrameSelector,
    FAbcMeshSample*& Sample, const ESampleReadFlags ReadFlags, const bool bFirstFrame
) {
  ...
  if (EnumHasAnyFlags(ReadFlags, ESampleReadFlags::Velocities)) {
    ...
    if (bVelocitiesAvailable) {
      ...
      bRetrievalResult &=
          RetrieveTypedAbcData<Alembic::Abc::V3fArraySamplePtr, FVector>(VelocitiesSample, Sample->Velocities);
    }
  }

  TArray<uint32> FaceCounts;
  if (EnumHasAnyFlags(
          ReadFlags, ESampleReadFlags::Indices | ESampleReadFlags::UVs | ESampleReadFlags::Normals |
                         ESampleReadFlags::Colors | ESampleReadFlags::MaterialIndices
      )) {
  ...
  if (bFirstFrame) {
    ...
    if (Result) {
      ...
      TSharedRef<FTokenizedMessage> Message = FTokenizedMessage::Create(
          EMessageSeverity::Error, FText::Format(
                                       LOCTEXT(
                                           "FoundNGon",
                                           "Unable to import mesh due to a face consisting of {0} vertices, expecting "
                                           "triangles (3) or quads (4)."
                                       ),
                                       FText::FromString(FString::FromInt((*Result)))
                                   )
      );
  ...
  if (EnumHasAnyFlags(ReadFlags, ESampleReadFlags::UVs)) {
    ...
    if (UVCoordinateParameter.valid()) {
      ReadUVSetData(
          UVCoordinateParameter, FrameSelector, Sample->UVs[0], Sample->Indices, bNeedsTriangulation, FaceCounts,
          Sample->Vertices.Num()
      );
    } else {
      ...
      for (int32 GeomParamIndex = 0; GeomParamIndex < NumGeomParams; ++GeomParamIndex) {
        ...
        if (Alembic::AbcGeom::IV2fGeomParam::matches(PropertyHeader)) {
          if (Sample->NumUVSets < MAX_TEXCOORDS) {
            UVSetProperty = Alembic::AbcGeom::IV2fGeomParam(GeomParams, PropertyHeader.getName());
            ReadUVSetData(
                UVSetProperty, FrameSelector, Sample->UVs[Sample->NumUVSets], Sample->Indices, bNeedsTriangulation,
                FaceCounts, Sample->Vertices.Num()
            );
            ++Sample->NumUVSets;
          } else {
            TSharedRef<FTokenizedMessage> Message = FTokenizedMessage::Create(
                EMessageSeverity::Warning,
                LOCTEXT("MaxUVs", "More than 4 UV sets found. The remaining UV sets will be ignored.")
            );
            FAbcImportLogger::AddImportMessage(Message);
            break;
          }
  ...
  if (EnumHasAnyFlags(ReadFlags, ESampleReadFlags::Normals)) {
    ...
    if (bNormalsAvailable && (!bConstantNormals || (bConstantNormals && bFirstFrame))) {
      ...
      if (Sample->Normals.Num() > 0) {
        ...
        if (bIndexedNormals) {
          Alembic::Abc::UInt32ArraySamplePtr NormalIndiceSample =
              NormalParameter.getIndexProperty().getValue(FrameSelector);
          ...
          ProcessVertexAttributeArray(
              Sample->Indices, FaceCounts, bNeedsTriangulation, Sample->Vertices.Num(), Sample->Normals
          );
        }

        // Make sure the normals from the Alembic are really normalized
        ParallelFor(Sample->Normals.Num(), [&Sample](int32 Index) { Sample->Normals[Index].Normalize(); });
      } else if (bFirstFrame) {
        TSharedRef<FTokenizedMessage> Message = FTokenizedMessage::Create(
            EMessageSeverity::Warning,
            LOCTEXT("NoNormals", "Normals were supposed to be available but none were found.")
        );
        FAbcImportLogger::AddImportMessage(Message);
      }
    }
  ...
  if (EnumHasAnyFlags(ReadFlags, ESampleReadFlags::Colors)) {
    ...
    if (Color3Property.valid()) {
      ...
      if (bIndexedColors) {
        Alembic::Abc::UInt32ArraySamplePtr ColorIndiceSample =
            Color3Property.getIndexProperty().getValue(FrameSelector);
        ...
        ProcessVertexAttributeArray(
            Sample->Indices, FaceCounts, bNeedsTriangulation, Sample->Vertices.Num(), Sample->Colors
        );
      }
      ...
      if (bIndexedColors) {
        Alembic::Abc::UInt32ArraySamplePtr ColorIndiceSample =
            Color4Property.getIndexProperty().getValue(FrameSelector);
  ...
  if (EnumHasAnyFlags(ReadFlags, ESampleReadFlags::MaterialIndices)) {
    ...
    Sample->NumMaterials = GenerateMaterialIndicesFromFaceSets(
        *const_cast<Alembic::AbcGeom::IPolyMeshSchema*>(&Schema), FrameSelector, Sample->MaterialIndices
    );
...
void AbcImporterUtilities::ReadUVSetData(
  ...
  if (bIndexedUVs) {
    Alembic::Abc::UInt32ArraySamplePtr UVIndiceSample =
        UVCoordinateParameter.getIndexProperty().getValue(FrameSelector);
...
void AbcImporterUtilities::GenerateSmoothingGroupsIndices(FAbcMeshSample* MeshSample, float HardEdgeAngleThreshold) {
      ...
      if (VertexInformations.Num() == 0) {
        ...
        for (int32 VertexInfoIndex = 0; VertexInfoIndex < VertexInformations.Num(); ++VertexInfoIndex) {
          ...
          if (*StoredFaceIndex == FaceIndex) {
            ...
            // If the VertexIndices are the same but the FaceIndex differs we found two faces that share at least one
            // vertex, thus add them to the TouchFaces map
  ...
  GenerateSmoothingGroups(
      TouchingFaces, FaceNormals, MeshSample->SmoothingGroupIndices, MeshSample->NumSmoothingGroups,
...
void AbcImporterUtilities::CalculateNormals(FAbcMeshSample* Sample) {
  ...
  for (uint32 FaceIndex = 0; FaceIndex < NumFaces; ++FaceIndex) {
    ...
    FVector N =
        FVector::CrossProduct((VertexPositions[0] - VertexPositions[1]), (VertexPositions[0] - VertexPositions[2]));
    N.Normalize();
...
void AbcImporterUtilities::CalculateNormalsWithSmoothingGroups(
    FAbcMeshSample* Sample, const TArray<uint32>& SmoothingMasks, const uint32 NumSmoothingGroups
) {
  ...
  for (uint32 FaceIndex = 0; FaceIndex < NumFaces; ++FaceIndex) {
    ...
    for (int32 Index = 0; Index < 3; ++Index) {
      Sample->Normals[FaceOffset + Index] = SmoothingGroupVertexNormals.FindChecked(
          TPair<uint32, uint32>(SmoothingGroup, Sample->Indices[FaceOffset + Index])
      );
...
void AbcImporterUtilities::ComputeTangents(
    FAbcMeshSample* Sample, bool bIgnoreDegenerateTriangles, IMeshUtilities& MeshUtilities
) {
  ...
  MeshUtilities.CalculateMikkTSpaceTangents(
      Sample->Vertices, Sample->Indices, Sample->UVs[0], Sample->Normals, bIgnoreDegenerateTriangles, Sample->TangentX,
      Sample->TangentY
  );
}
  ...
  for (const FAbcMeshSample* Sample : Samples) {
    ...
    if (Sample->NumUVSets >= MergedSample->NumUVSets) {
      for (uint32 UVIndex = 1; UVIndex < Sample->NumUVSets; ++UVIndex) {
        const int32 NumMissingUVs =
            (MergedSample->UVs[0].Num() - MergedSample->UVs[UVIndex].Num()) - Sample->UVs[UVIndex].Num();
    ...
    ensureMsgf(
        MaterialIndicesOffset == SmoothingGroupIndicesOffset,
        TEXT("Material and smoothing group indice count should match")
    );
...
void AbcImporterUtilities::GetHierarchyForObject(
    const Alembic::Abc::IObject& Object, TDoubleLinkedList<Alembic::AbcGeom::IXform>& Hierarchy
) {
  ...
  while (Parent.valid()) {
    // Only if the Object is of type IXform we need to store it in the hierarchy (since we only need them for matrix
    // animation right now)
...
void AbcImporterUtilities::PropogateMatrixTransformationToSample(FAbcMeshSample* Sample, const FMatrix& Matrix) {
  for (FVector& Position : Sample->Vertices) {
    Position = (FVector4)Matrix.TransformPosition((FVector)Position);
  }

  for (FVector& Velocity : Sample->Velocities) {
    Velocity = (FVector4)Matrix.TransformVector((FVector)Velocity);
  }

  // TODO could make this a for loop and combine the transforms
  for (FVector& Normal : Sample->Normals) {
    Normal = (FVector4)Matrix.TransformVector((FVector)Normal);
    Normal.Normalize();
  }

  for (FVector& TangentX : Sample->TangentX) {
    TangentX = (FVector4)Matrix.TransformVector((FVector)TangentX);
    TangentX.Normalize();
  }

  for (FVector& TangentY : Sample->TangentY) {
    TangentY = (FVector4)Matrix.TransformVector((FVector)TangentY);
    TangentY.Normalize();
  }
}

void AbcImporterUtilities::GenerateDeltaFrameDataMatrix(
  ...
  for (uint32 VertexIndex = 0; VertexIndex < NumVertices; ++VertexIndex) {
    ...
    const FVector AverageDifference =
        ((FVector)AverageVertexData[VertexIndex + AverageVertexOffset] + SamplePositionOffset) -
        (FVector)FrameVertexData[VertexIndex];
  ...
  for (uint32 Index = 0; Index < NumIndices; ++Index) {
    ...
    const FVector AverageNormal =
        (FVector)AverageNormalData[Index + AverageIndexOffset] - (FVector)FrameNormalData[Index];
...
void AbcImporterUtilities::GenerateCompressedMeshData(
    FCompressedAbcData& CompressedData, const uint32 NumUsedSingularValues, const uint32 NumSamples,
    const TArrayView<float>& BasesMatrix, const TArrayView<float>& NormalsBasesMatrix,
    const TArray<float>& BasesWeights, const float SampleTimeStep, const float StartTime
) {
  ...
  for (uint32 BaseIndex = 0; BaseIndex < NumUsedSingularValues; ++BaseIndex) {
    ...
    // Use original number of singular values to index into the array (otherwise we would be reading incorrect data if
    // NumUsedSingularValues != the original number
...
void AbcImporterUtilities::CalculateNewStartAndEndFrameIndices(
    const float FrameStepRatio, int32& InOutStartFrameIndex, int32& InOutEndFrameIndex
) {
...
bool AbcImporterUtilities::AreVerticesEqual(const FSoftSkinVertex& V1, const FSoftSkinVertex& V2) {
  ...
  for (int32 UVIdx = 0; UVIdx < 1 /*MAX_TEXCOORDS*/; ++UVIdx) {
    if (FMath::Abs(V1.UVs[UVIdx].X - V2.UVs[UVIdx].X) > (1.0f / 1024.0f)) return false;

    if (FMath::Abs(V1.UVs[UVIdx].Y - V2.UVs[UVIdx].Y) > (1.0f / 1024.0f)) return false;
  }
  ...
  if (FMath::Abs(N1.X - N2.X) > THRESH_NORMALS_ARE_SAME || FMath::Abs(N1.Y - N2.Y) > THRESH_NORMALS_ARE_SAME ||
      FMath::Abs(N1.Z - N2.Z) > THRESH_NORMALS_ARE_SAME) {
    return false;
  }
...
void AbcImporterUtilities::ApplyConversion(
    FAbcMeshSample* InOutSample, const FDoodleAbcConversionSettings& InConversionSettings,
    const bool bShouldInverseBuffers
) {
  if (InConversionSettings.bFlipV || InConversionSettings.bFlipU) {
    ...
    for (uint32 UVIndex = 0; UVIndex < InOutSample->NumUVSets; ++UVIndex) {
      for (FVector2D& UV : InOutSample->UVs[UVIndex]) {
        UV = UVOffset + FVector2D(UVMatrix.TransformPoint(FVector2D(UV)));
      }
    }
  }

  // Calculate conversion matrix
  const FMatrix Matrix = FScaleMatrix::Make(InConversionSettings.Scale) *
                         FRotationMatrix::Make(FQuat::MakeFromEuler(InConversionSettings.Rotation));
...
bool AbcImporterUtilities::IsObjectVisible(
    const Alembic::Abc::IObject& Object, const Alembic::Abc::ISampleSelector FrameSelector
) {
  ...
  if (CompoundProperty.getPropertyHeader(Alembic::AbcGeom::kVisibilityPropertyName)) {
    visibilityProperty =
        Alembic::AbcGeom::IVisibilityProperty(CompoundProperty, Alembic::AbcGeom::kVisibilityPropertyName);
  }
  ...
  if (visibilityValue == Alembic::AbcGeom::kVisibilityHidden) return false;
...
FBoxSphereBounds AbcImporterUtilities::ExtractBounds(Alembic::Abc::IBox3dProperty InBoxBoundsProperty) {
  ...
  if (InBoxBoundsProperty.valid()) {
    ...
    for (int32 SampleIndex = 0; SampleIndex < NumSamples; ++SampleIndex) {
      ...
      const FBoxSphereBounds ConvertedBounds(
          FVector(BoundCenter.x, BoundCenter.y, BoundCenter.z),
          FVector(BoundSize.x * 0.5f, BoundSize.y * 0.5f, BoundSize.z * 0.5f), (const float)BoundSize.length() * 0.5f
      );
...
void AbcImporterUtilities::ApplyConversion(
    FMatrix& InOutMatrix, const FDoodleAbcConversionSettings& InConversionSettings
) {
  // Calculate conversion matrix
  const FMatrix ConversionMatrix = FScaleMatrix::Make(InConversionSettings.Scale) *
                                   FRotationMatrix::Make(FQuat::MakeFromEuler(InConversionSettings.Rotation));
  InOutMatrix = InOutMatrix * ConversionMatrix;
}

void AbcImporterUtilities::ApplyConversion(
    FBoxSphereBounds& InOutBounds, const FDoodleAbcConversionSettings& InConversionSettings
) {
  // Calculate conversion matrix
  const FMatrix ConversionMatrix = FScaleMatrix::Make(InConversionSettings.Scale) *
                                   FRotationMatrix::Make(FQuat::MakeFromEuler(InConversionSettings.Rotation));
...
void AbcImporterUtilities::ApplyConversion(
    TArray<FMatrix>& InOutMatrices, const FDoodleAbcConversionSettings& InConversionSettings
) {
  // Calculate conversion matrix
  const FMatrix ConversionMatrix = FScaleMatrix::Make(InConversionSettings.Scale) *
                                   FRotationMatrix::Make(FQuat::MakeFromEuler(InConversionSettings.Rotation));
...
void AbcImporterUtilities::GeometryCacheDataForMeshSample(
    FGeometryCacheMeshData& OutMeshData, const FAbcMeshSample* MeshSample, const uint32 MaterialOffset,
    const float SecondsPerFrame, const bool bUseVelocitiesAsMotionVectors, const bool bStoreImportedVertexNumbers
) {
  OutMeshData.BoundingBox   = MeshSample->Vertices.Num() ? FBox(MeshSample->Vertices) : FBox(EForceInit::ForceInit);
  ...
  for (int32 TriangleIndex = 0; TriangleIndex < NumTriangles; ++TriangleIndex) {
    ...
    for (int32 VertexIndex = 0; VertexIndex < 3; ++VertexIndex) {
      ...
      if (bHasTangents) {
        ...
        OutMeshData.TangentsZ[CornerIndex].Vector.W = GetBasisDeterminantSignByte(
            MeshSample->TangentX[CornerIndex], MeshSample->TangentY[CornerIndex], MeshSample->Normals[CornerIndex]
        );
      } else {
...
void AbcImporterUtilities::MergePolyMeshesToMeshData(
    int32 FrameIndex, int32 FrameStart, float SecondsPerFrame, bool bUseVelocitiesAsMotionVectors,
    ...
    if (PolyMesh->bShouldImport) {
      ...
      if (PolyMesh->GetVisibility(FrameIndex)) {
        ...
        if (PolyMesh->FaceSetNames.Num() == 0) {
          FMemory::Memzero(
              MergedSample.MaterialIndices.GetData() + Offset,
              (MergedSample.MaterialIndices.Num() - Offset) * sizeof(int32)
          );
        } else {
          for (int32 Index = Offset; Index < MergedSample.MaterialIndices.Num(); ++Index) {
            ...
            if (PolyMesh->FaceSetNames.IsValidIndex(MaterialIndex - MaterialIndexOffset)) {
              int32 FaceSetMaterialIndex =
                  UniqueFaceSetNames.IndexOfByKey(PolyMesh->FaceSetNames[MaterialIndex - MaterialIndexOffset]);
  ...
  AbcImporterUtilities::GeometryCacheDataForMeshSample(
      MeshData, &MergedSample, 0, SecondsPerFrame, bUseVelocitiesAsMotionVectors, bStoreImportedVertexNumbers
  );
}

UMaterialInterface* AbcImporterUtilities::RetrieveMaterial(
    FAbcFile& AbcFile, const FString& MaterialName, UObject* InParent, EObjectFlags Flags
) {
