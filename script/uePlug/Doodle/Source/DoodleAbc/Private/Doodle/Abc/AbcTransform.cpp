...
FAbcTransform::FAbcTransform(
    const Alembic::AbcGeom::IXform& InTransform, const FAbcFile* InFile, IAbcObject* InParent /*= nullptr*/
)
    : IAbcObject(InTransform, InFile, InParent), Transform(InTransform), Schema(InTransform.getSchema()) {
...
void FAbcTransform::SetFrameAndTime(
  ...
  if (TargetIndex != INDEX_NONE) {
    ...
    if (!bConstantIdentity && !bConstant) {
      Alembic::Abc::ISampleSelector SampleSelector =
          AbcImporterUtilities::GenerateAlembicSampleSelector<double>(InTime);
      Alembic::AbcGeom::XformSample MatrixSample;
      Schema.get(MatrixSample, SampleSelector);
