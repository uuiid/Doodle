...
void FAbcImportSettingsCustomization::CustomizeDetails(IDetailLayoutBuilder& LayoutBuilder) {
  TSharedRef<IPropertyHandle> ImportType =
      LayoutBuilder.GetProperty(GET_MEMBER_NAME_CHECKED(UDoodleAbcImportSettings, ImportType));

  uint8 EnumValue;
  ImportType->GetValue(EnumValue);
  IDetailCategoryBuilder& CompressionBuilder = LayoutBuilder.EditCategory("Compression");
  CompressionBuilder.SetCategoryVisibility(EnumValue == (uint8)EDoodleAlembicImportType::Skeletal);

  IDetailCategoryBuilder& GeomCacheBuilder = LayoutBuilder.EditCategory("GeometryCache");
  GeomCacheBuilder.SetCategoryVisibility(EnumValue == (uint8)EDoodleAlembicImportType::GeometryCache);

  FSimpleDelegate OnImportTypeChangedDelegate =
      FSimpleDelegate::CreateSP(this, &FAbcImportSettingsCustomization::OnImportTypeChanged, &LayoutBuilder);
  ImportType->SetOnPropertyValueChanged(OnImportTypeChangedDelegate);

  TArray<TWeakObjectPtr<UObject>> Objects;
  LayoutBuilder.GetObjectsBeingCustomized(Objects);

  TWeakObjectPtr<UObject>* SettingsObject = Objects.FindByPredicate([](const TWeakObjectPtr<UObject>& Object) {
    return Object->IsA<UDoodleAbcImportSettings>();
  });

  UDoodleAbcImportSettings* CurrentSettings =
      SettingsObject ? Cast<UDoodleAbcImportSettings>(SettingsObject->Get()) : nullptr;

  if (CurrentSettings && CurrentSettings->bReimport) {
    UEnum* ImportTypeEnum = StaticEnum<EDoodleAlembicImportType>();
    static FText RestrictReason =
        NSLOCTEXT("AlembicImportFactory", "ReimportRestriction", "Unable to change type while reimporting");
...
void FAbcSamplingSettingsCustomization::CustomizeChildren(
  ...
  for (uint32 ChildIndex = 0; ChildIndex < NumChildren; ++ChildIndex) {
    ...
    // if (ChildHandle->GetProperty()->GetFName() == GET_MEMBER_NAME_CHECKED(FDoodleAbcSamplingSettings, SamplingType))
    // {
    ...
    Property.Visibility(TAttribute<EVisibility>::Create(TAttribute<EVisibility>::FGetter::CreateSP(
        this, &FAbcSamplingSettingsCustomization::ArePropertiesVisible, EnumCondition
    )));
  }
}
...
void FAbcCompressionSettingsCustomization::CustomizeChildren(
    TSharedRef<IPropertyHandle> StructPropertyHandle, IDetailChildrenBuilder& StructBuilder,
    IPropertyTypeCustomizationUtils& StructCustomizationUtils
) {
  uint32 NumChildren;
  ...
  for (uint32 ChildIndex = 0; ChildIndex < NumChildren; ++ChildIndex) {
    ...
    // if (ChildHandle->GetProperty()->GetFName() == GET_MEMBER_NAME_CHECKED(FDoodleAbcCompressionSettings,
    // BaseCalculationType)) {
    ...
    int32 EnumCondition                     = ChildHandle->GetIntMetaData(EditConditionName);
    Property.Visibility(TAttribute<EVisibility>::Create(TAttribute<EVisibility>::FGetter::CreateSP(
        this, &FAbcCompressionSettingsCustomization::ArePropertiesVisible, EnumCondition
    )));
  }
}
...
void FAbcConversionSettingsCustomization::CustomizeChildren(
    TSharedRef<IPropertyHandle> StructPropertyHandle, IDetailChildrenBuilder& StructBuilder,
    IPropertyTypeCustomizationUtils& StructCustomizationUtils
) {
  FSimpleDelegate OnPresetChanged =
      FSimpleDelegate::CreateSP(this, &FAbcConversionSettingsCustomization::OnConversionPresetChanged);
  FSimpleDelegate OnValueChanged =
      FSimpleDelegate::CreateSP(this, &FAbcConversionSettingsCustomization::OnConversionValueChanged);
