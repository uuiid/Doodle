...
static void ConvertArrayToEigenMatrix(
    const TArray<float>& InArray, const int32 InRows, const int32 InColumns, Eigen::MatrixXf& OutMatrix
) {
...
static void ConvertEigenMatrixToArray(
    const Eigen::MatrixXf& InMatrix, TArray<float>& OutArray, uint32& OutColumns, uint32& OutRows
) {
...
static void PerformSVD(
    const Eigen::MatrixXf& InMatrix, Eigen::MatrixXf& OutU, Eigen::MatrixXf& OutV, Eigen::MatrixXf& OutS
) {
...
/** Performs Singular value decomposition on the given matrix and returns respective the calculated U, V and S Matrix */
static void PerformSVD(
    const TArray<float>& InMatrix, const int32 InRows, const int32 InColumns, TArray<float>& OutU, TArray<float>& OutV,
    TArray<float>& OutS
) {
