...
template <typename T>
bool IsType(const Alembic::Abc::MetaData& MetaData) {
...
uint32 GenerateMaterialIndicesFromFaceSets(
...
bool RetrieveTypedAbcData(T InSampleDataPtr, TArray<U>& OutDataArray) {
  ...
  if (NumEntries) {
    ...
    // Ensure that the destination and source data size corresponds (otherwise we will end up with an invalid memcpy and
    // means we have a type mismatch)
...
template <typename T>
void ExpandVertexAttributeArray(const TArray<uint32>& InIndices, TArray<T>& InOutArray) {
...
/** Triangulates the given (non-indexed) vertex attribute data buffer (assuming incoming data is quads or quad/triangle
 * mix) */
template <typename T>
void TriangulateVertexAttributeBuffer(const TArray<uint32>& InFaceCounts, TArray<T>& InOutData) {
...
template <typename T>
void ExpandPrimitiveAttributeArray(const TArray<uint32>& InFaceCounts, TArray<T>& InOutArray) {
  int32 NumIndices = 0;
  for (const uint32 NumIndicesForFace : InFaceCounts) {
...
template <typename T>
void ProcessVertexAttributeArray(
    const TArray<uint32>& InIndices, const TArray<uint32>& InFaceCounts, const bool bTriangulation,
    const uint32 NumVertices, TArray<T>& InOutArray
) {
...
/** Triangulates material indices according to the face counts (quads will have to be split up into two faces / material
 * indices)*/
...
FAbcMeshSample* GenerateAbcMeshSampleForFrame(
    const Alembic::AbcGeom::IPolyMeshSchema& Schema, const Alembic::Abc::ISampleSelector FrameSelector,
    const ESampleReadFlags ReadFlags, const bool bFirstFrame = false
);
...
bool GenerateAbcMeshSampleDataForFrame(
    const Alembic::AbcGeom::IPolyMeshSchema& Schema, const Alembic::Abc::ISampleSelector FrameSelector,
...
void CalculateNormalsWithSmoothingGroups(
...
template <typename T>
float RetrieveTimeForFrame(T& Schema, const uint32 FrameIndex) {
...
template <typename T>
void GetStartTimeAndFrame(T& Schema, float& StartTime, uint32& StartFrame) {
  checkf(Schema.valid(), TEXT("Invalid Schema"));
  Alembic::AbcCoreAbstract::TimeSamplingPtr TimeSampler   = Schema.getTimeSampling();
...
template <typename T>
void GetStartTimeAndFrame(T& Schema, float& StartTime, int32& StartFrame) {
  checkf(Schema.valid(), TEXT("Invalid Schema"));
  Alembic::AbcCoreAbstract::TimeSamplingPtr TimeSampler   = Schema.getTimeSampling();
...
void GenerateCompressedMeshData(
...
void CalculateNewStartAndEndFrameIndices(
...
void ApplyConversion(TArray<FMatrix>& InOutMatrices, const FDoodleAbcConversionSettings& InConversionSettings);

/** Applies user/preset conversion to the given matrices */
void ApplyConversion(FMatrix& InOutMatrix, const FDoodleAbcConversionSettings& InConversionSettings);
...
void ApplyConversion(FBoxSphereBounds& InOutBounds, const FDoodleAbcConversionSettings& InConversionSettings);
/** Returns whether or not the given Object is visible according at the retrieved frame using SampleSelector (this
 * includes parent Objects) */
bool IsObjectVisible(const Alembic::Abc::IObject& Object, const Alembic::Abc::ISampleSelector FrameSelector);
/** Returns whether or not the Objects visibility property is constant across the entire sequence (this includes parent
 * Objects) */
...
void GeometryCacheDataForMeshSample(
 ...
 * @param PreviousNumVertices	The number of vertices in the merged PolyMeshes, used to determine if its topology is
 * constant between 2 frames
 ...
 */
void MergePolyMeshesToMeshData(
...
UMaterialInterface* RetrieveMaterial(
