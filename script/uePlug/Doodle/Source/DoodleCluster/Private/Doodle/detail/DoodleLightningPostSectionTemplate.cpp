  ...
  virtual void Execute(
    ...
    for (auto i : Player.FindBoundObjects(Operand)) {
      if (UObject* L_Object = i.Get();
        ...
        const float L_Time_Range_Size       = Context.GetFrameNumberRange().Size<FFrameNumber>().Value;
        const auto L_Time                   = L_Current_Time / L_Time_Range_Size;
        const auto L_Curve_Time             = SectionTemplate->Params->IntensityCurve->GetFloatValue(L_Time);

        L_LightingPost->GetLightComponent()->SetIntensity(L_Current_Time * L_LightingPost->IntensityMultiplier);
        L_LightingPost->Settings.ColorSaturation[3] = L_Curve_Time * L_LightingPost->SaturationMultiplier;
        L_LightingPost->Settings.ColorContrast[3]   = L_Curve_Time * L_LightingPost->ContrastMultiplier;
      }
    }
  }
