...
DECLARE_CYCLE_STAT(TEXT("Doodle Event Track Token Execute"),
                   MovieSceneEval_EventTrack_TokenExecute_Doodle,
        ...
        IMovieScenePlayer &Player) override
    {
        ...
        for (auto i : Player.FindBoundObjects(Operand))
        {
            if (UObject *L_Object = i.Get())
            {
                ...
                for (auto j : Player.FindBoundObjects(SectionTemplate->Params->DoodleLockAtObject.GetGuid(),
