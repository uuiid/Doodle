...
void toolkit::installUePath(const std::string &path) {
  ...
  if (path.empty()) {
    ...
    ue_path = QInputDialog::getItem(nullptr, "选择安装ue插件的版本", "路径", list).toStdString();
    if (ue_path.empty()) return;
    //判断并选择来源
    if (std::regex_search(ue_path.generic_string(), std::regex{"4.25"})) {
      sourePath = doCore::coreSet::getSet().program_location().parent_path() / "plug/uePlug/4.25/Plugins/Doodle";
    } else if (std::regex_search(ue_path.generic_string(), std::regex{"4.26"})) {
      sourePath = doCore::coreSet::getSet().program_location().parent_path() / "plug/uePlug/4.26/Plugins/Doodle";
    } else {
      QMessageBox::warning(nullptr, QString::fromUtf8("注意"), QString::fromUtf8("没有编译这个版本的插件"));
      return;
    }
    ...
    auto ue_version = QInputDialog::getItem(nullptr, "选择安装ue插件的版本", "路径", list).toFloat();
    if(ue_version == 4.25){
      sourePath = doCore::coreSet::getSet().program_location().parent_path() / "plug/uePlug/4.25/Plugins/Doodle";
    } else{
      sourePath = doCore::coreSet::getSet().program_location().parent_path() / "plug/uePlug/4.26/Plugins/Doodle";
