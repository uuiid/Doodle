...
void ProjectWidget::init_() {
  ...
  ////添加light页面
  //auto light_parent = new QWidget();
  //addTab(light_parent, QString("Light"));
  //lightInit(light_parent);
  //setTabText(2, tr("灯光"));
}
...
void ProjectWidget::lightInit(QWidget* parent) {
  ...
  //auto dep_widght =  new AssDepWidget(parent);
  //dep_widght->setObjectName("p_file_class_ass_widget_");
  //dep_widght->setModel(p_ass_dep_model_);

  //auto ass_cl_widght = new assClassWidget(parent);
  //ass_cl_widght->setObjectName("p_ass_class_widget_");
  //ass_cl_widght->setModel(p_ass_class_model_);
  //connect(dep_widght, &AssDepWidget::initEmit,
  //  p_ass_class_model_,  &assClassModel::init);

  ////设置基本布局
  //auto eps_layout = new QHBoxLayout(parent);
  //eps_layout->setSpacing(3);
