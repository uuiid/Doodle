...
void episodesintDelegate::setEditorData(QWidget *editor,
                                        const QModelIndex &index) const {
...
void episodesintDelegate::updateEditorGeometry(
...
shotEpsListWidget::shotEpsListWidget(QWidget *parent)
    : QListView(parent), p_episodesListDelegate(nullptr), p_eps_Menu(nullptr) {
  ...
  connect(this, &shotEpsListWidget::clicked, this,
          &shotEpsListWidget::_doodle_episodes_emit);
}
...
void shotEpsListWidget::contextMenuEvent(QContextMenuEvent *event) {
  ...
  connect(add_eps, &QAction::triggered, this,
          &shotEpsListWidget::insertEpisodes);
...
<<<<<<< HEAD
    ...
    connect(k_deleteEpsiodes, &QAction::triggered,
            [=]() { this->deleteEpsiodes(); });
...
void shotEpsListWidget::_doodle_episodes_emit(const QModelIndex &index) {
  doCore::coreDataManager::get().setEpisodesPtr(
      index.data(Qt::UserRole).value<doCore::episodesPtr>());
  initEmit();
}
