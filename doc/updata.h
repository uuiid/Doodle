 ...
 * @li 更新导出csv表格美化
 * @li 更加准确的文件存在判断
 * @li maya插件 将硬链接不解析为ip路径
 * @li 修正图片合成拍屏功能
 * @li ue4_plug 火光复制功能添加
 * @li @b 火光属性记录动画帧元数据
 * @li @b 灯光球属性记录动画帧元数据
 * @li maya插件 右键选颜色
 ...
 * @li ue4 更新ue4 导入文件方法(未发布)
 * @li ue4 添加新的关卡创建命令
 * @li ue4 添加新的音序器创建命令
 * @li ue4 添加新的音序器内相机创建命令和自动化范围
 * @li exe 寻找图标问题修复
 * @li exe 修复导出时的时间计算方式
 * @li exe 添加文件组件的其他属性gui设置
 * @li exe 更新选中颜色(加强)
 * @li exe 绝对路径选项
 * @li exe 添加时间过滤器
 * @li exe 去除编辑过滤器按钮
 * @li exe 添加文件选择器中的路径输入框
 * @li exe 添加了编辑项目分类批量编辑的方法
 ...
 * @li 添加了季数列
 * @li 更新了排序方法
 * @li ue4 插件更新了灯光配置actor
 * @li ue4 插件更新了actor方便拖拽工厂
 * @li ue4 插件灯光配置actor添加鲜明的图标
 * @li 修复了图片加载工具多线程初始化问题
 * @li 优化了路径计算
 * @li 批量编辑时细粒度的更新
 * @li 添加了备注的修改方法
 * @li 添加了重要性编辑方法
 ...
 * @li 文件对话框输入默认值更改
 *
 * @subsubsection updata_log_3420 版本3.4.20
 * @li ue4 tool 更新导入设置分类
 * @li 美化界面字体
 * @li 添加了文件列表渲染切换按钮
 * @li 添加了文件列表列渲染模式
 * @li 更新了映射磁盘项目的小工具
 ...
 * @li 资产过滤器添加多选
 *
 * @subsubsection updata_log_3421 版本3.4.21
 * @li 更新检查拖入图片时不检查正则表达式
 * @li 添加了ue4导入模块面板
 * @li 添加了ue4导入功能
 * @li 添加了ue4 导入筛选正则表达式
 * @li 添加了ue4导入格式化结果方法
 ...
 * @li 修正maya插件加载错误
 *
 * @subsubsection updata_log_3424 版本3.4.24
 * @li 记录了窗口的打开状态
 * @li 修正了窗口打开时的状态保存
 * @li 添加了保存窗口的设置
 * @li 初步添加窗口布局管理
 * @li 添加提取剧本功能
 * @li 排序资产库功能添加
 * @li 去除相对和绝对路径的选项, 更加智能的判断(优先相对, 如果相对失败则使用绝对路径)
 * @li 层级标签过滤器调整
 * @li 修正maya 插件导出fbx时验证有效失误
 *
 * @subsubsection updata_log_3425 版本3.4.25
 * @li 添加字幕处理
 * @li 字幕处理: 去除所有标点(不包括括号和冒号)
 * @li 字幕处理: 去除括号内内容
 * @li 字幕处理: 去除冒号前内容
 * @li 字幕处理: 切断行
 * @li 字幕处理: 切断行大小设置
