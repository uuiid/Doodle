...
BOOST_AUTO_TEST_CASE(rebuild_all_tables_on_real_db) {
  ...
  app_base l_app{};
  {
    ...
    BOOST_TEST_MESSAGE(fmt::format(
        "重建前: entity={} task={} entity_fts_docsize={} 表数={}", l_entity_before, l_task_before, l_docsize_before,
        l_counts_before.size()
    ));
    ...
    // 2. 外键形态与违规.
    //    库已是 v28, 重建**不会**再新增任何外键 —— 此前"重建会补上老库缺的外键"那套
    //    按 key 逐个比较违规数的逻辑已经没有对象, 已随迁移专用用例一起退役.
    //    这里改断言更直接的性质: 重建不得制造任何外键违规.
    auto l_fk_after = fk_by_child_parent(l_session);
    for (const auto& [l_key, l_value] : l_fk_after)
      BOOST_TEST_MESSAGE(fmt::format("重建后外键违规: {} = {}", l_key, l_value));
    BOOST_TEST(l_fk_after.empty());

    // 修正后的外键目标必须被重建保持: comment.object_id -> task(uuid), 不能退回 entity(uuid)
    ...
    BOOST_TEST(fk_by_child_parent(l_session).empty());
    BOOST_TEST(scalar_text(l_session, "PRAGMA integrity_check;") == "ok");
  }
