...
void test_fun3(boost::system::error_code in_code) {
  ...
  l_maya_exe->async_run_maya(
      entt::handle{*g_reg(), g_reg()->create()}, k_arg, maya_to_exe_file{FSys::path{l_out_path}}
  );
}
...
void test_fun2(boost::system::error_code in_code) {
  ...
  l_handle.emplace<database>();
  l_handle.emplace<assets_file>().path_attr("C:/sy/LianQiShiWanNian_8/6-moxing/Ch/JDCh_05/Ch426A/BuJiaBan_UE5");
