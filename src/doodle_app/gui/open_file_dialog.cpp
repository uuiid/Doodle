...
class file_panel::path_info {
 public:
  path_info() : path(), is_dir(), show_name(), size(), size_string(), last_time(), has_select(){};

  path_info &init(const FSys::path &in_path) {
    path      = in_path;
    is_dir    = is_directory(in_path);

    show_name = fmt::format(
  ...
  operator bool() const { return !show_name.empty() && !path.empty(); }
  bool operator==(const path_info &in_rhs) const { return path == in_rhs.path; }
  bool operator!=(const path_info &in_rhs) const { return !(in_rhs == *this); }
  bool operator<(const path_info &in_rhs) const { return std::tie(path) < std::tie(in_rhs.path); }
  bool operator>(const path_info &in_rhs) const { return in_rhs < *this; }
  bool operator<=(const path_info &in_rhs) const { return !(in_rhs < *this); }
  bool operator>=(const path_info &in_rhs) const { return !(*this < in_rhs); }
};
...
file_panel::default_pwd::default_pwd() : default_pwd(FSys::current_path()) {}
file_panel::default_pwd::default_pwd(FSys::path in_pwd) : pwd(std::move(in_pwd)) {}

file_panel::file_panel(const dialog_args &in_args) : p_i(std::make_unique<impl>()) {
...
void file_panel::init() { this->scan_director(p_i->p_pwd); }
...
void file_panel::scan_director(const FSys::path &in_dir) {
  ...
  if (!FSys::is_directory(in_dir)) return;
  ...
  try {
    p_i->path_list =
        ranges::make_subrange(FSys::directory_iterator{in_dir}, FSys::directory_iterator{}) |
        ...
        ranges::views::filter([](const auto &in_info) -> bool { return in_info; }) |  /// 过滤不符合过滤器的
        ranges::views::filter([this](const path_info &in_info) -> bool {
          /// 进行目录过滤
          if (!p_i->p_flags_[0]) {
            if (p_i->filter_list.show_str != "*.*") {
              return in_info.is_dir || in_info.path.extension() == p_i->filter_list.show_str;
            }
          } else {
...
void file_panel::sort_file_attr(sort_by in_sort_by, bool in_reverse) {
  p_i->path_list |= ranges::actions::sort([&](const path_info &in_l, const path_info &in_r) -> bool {
    bool result;
    switch (in_sort_by) {
      case sort_by::name:
  ...
  /// \brief 将目录和文件进行分区
  ranges::stable_partition(p_i->path_list, [](const path_info &in) -> bool { return in.is_dir; });
  if (in_reverse) ranges::reverse(p_i->path_list);
  p_i->select_index = 0;
}
  ...
  if (!edit) {
    if (!p_i->p_pwd.empty()) {
      ...
      if (imgui::Button(p_i->p_pwd.root_path().generic_string().c_str())) {
        p_i->begin_fun_list.emplace_back([this, in_path = p_i->p_pwd.root_path()]() { this->scan_director(in_path); });
      }
    }
    ...
    for (auto &k_p : p_i->p_pwd.relative_path()) {
      ...
      if (imgui::Button(fmt::format("{0}##{1}", k_p.generic_string(), k_i).c_str())) {
        ...
        p_i->begin_fun_list.emplace_back([this, in_path = k_r]() { this->scan_director(in_path); });
      }
      ++k_i;
    }
  } else {
    imgui::SameLine();
    if (ImGui::InputText(
            *p_i->edit_input.gui_name, &p_i->edit_input.data, ImGuiInputTextFlags_::ImGuiInputTextFlags_EnterReturnsTrue
        )) {
      p_i->edit_button.data = false;
      p_i->begin_fun_list.emplace_back([this]() { this->scan_director(p_i->edit_input.data); });
    }
  }
}
void file_panel::render_list_path() {
  static auto table_flags{
      ...
      ImGuiTableFlags_::ImGuiTableFlags_BordersOuter | ImGuiTableFlags_::ImGuiTableFlags_ScrollX |
      ImGuiTableFlags_::ImGuiTableFlags_ScrollY | ImGuiTableFlags_::ImGuiTableFlags_Sortable};
  dear::Table{"file list", 3, table_flags, ImVec2(0.0f, -ImGui::GetTextLineHeightWithSpacing() * 3)} && [&]() {
    /// \brief 设置题头元素
    ImGui::TableSetupColumn(
        "file name", ImGuiTableColumnFlags_DefaultSort | ImGuiTableColumnFlags_WidthFixed, 0.0f,
        magic_enum::enum_integer(sort_by::name)
    );
    ImGui::TableSetupColumn("size", ImGuiTableColumnFlags_WidthFixed, 0.0f, magic_enum::enum_integer(sort_by::size));
    ImGui::TableSetupColumn(
        "last write time", ImGuiTableColumnFlags_WidthFixed, 0.0f, magic_enum::enum_integer(sort_by::time)
    );
    ...
    if (auto *l_sorts_specs = dear::TableGetSortSpecs()) {
      if (l_sorts_specs->SpecsDirty) {
        this->sort_file_attr(
            magic_enum::enum_cast<sort_by>(boost::numeric_cast<std::int16_t>(l_sorts_specs->Specs[0].ColumnUserID))
                .value(),
    ...
    for (auto [l_index, k_p] : p_i->path_list | ranges::views::enumerate) {
      ...
      if (dear::Selectable(
              k_p.show_name, k_p.has_select, ImGuiSelectableFlags_SpanAllColumns | ImGuiSelectableFlags_AllowDoubleClick
          )) {
        set_select(l_index);
      }
      /// \brief 文件大小
...
void file_panel::set_select(std::size_t in_index) {
  ...
  if (ImGui::IsMouseDoubleClicked(ImGuiMouseButton_::ImGuiMouseButton_Left) && k_p.is_dir) {  /// \brief 双击函数
    p_i->begin_fun_list.emplace_back([=, in_path = k_p.path]() { this->scan_director(in_path); });
    ...
    if (p_i->p_flags_[2]) {
      ...
      if (k_io.KeyCtrl)
        ...
        ranges::for_each(
            p_i->path_list | ranges::views::slice(
                                 boost::numeric_cast<std::size_t>(std::min(in_index, p_i->select_index)),
                                 boost::numeric_cast<std::size_t>(
                                     std::min(std::max(in_index, p_i->select_index) + 1, p_i->path_list.size())
                                 )
                             ),
            [](path_info &in_attr) { in_attr.has_select = true; }
        );

      } else {
        ranges::for_each(p_i->path_list, [](auto &in) { in.has_select = false; });
        k_p.has_select = true;
      }
    } else {  /// \brief 单选
      ranges::for_each(p_i->path_list, [](auto &in) { in.has_select = false; });
...
void file_panel::render_buffer() { ImGui::InputText(*p_i->buffer.gui_name, &(p_i->buffer.data)); }
  ...
  dear::Combo{*p_i->filter_list.gui_name, p_i->filter_list.show_str.c_str()} && [&]() {
    for (auto &k_f : p_i->filter_list.data) {
      if (dear::Selectable(k_f)) {
        ...
        p_i->begin_fun_list.emplace_back([this]() { scan_director(p_i->p_pwd); });
      }
    }
  };
}
...
void file_panel::generate_buffer(std::size_t in_index) {
  if (ranges::any_of(p_i->path_list, [](const path_info &in) -> bool { return in.has_select; })) {
    auto l_size = ranges::count_if(p_i->path_list, [](const path_info &in) -> bool { return in.has_select; });
    if (l_size > 1) {
...
FSys::path file_panel::get_select() {
  FSys::path result{};
  if (std::any_of(p_i->path_list.begin(), p_i->path_list.end(), [](const path_info &in) { return in.has_select; }))
    result = p_i->path_list.at(p_i->select_index).path;
  else
    result = p_i->p_pwd;
...
std::vector<FSys::path> file_panel::get_selects() {
  ...
  result = p_i->path_list | ranges::views::filter([](const path_info &in_attr) -> bool { return in_attr.has_select; }) |
           ranges::views::transform([](const path_info &in_attr) { return in_attr.path; }) | ranges::to_vector;

  if (result.empty()) result.emplace_back(p_i->p_pwd);
