...
///=================================================================
class gui_strand_executor_service
    : public execution_context_service_base<gui_strand_executor_service> {
 public:
  class strand_impl {
   public:
    ~strand_impl();
   ...
   private:
    friend class gui_strand_executor_service;
    ...
    gui_strand_executor_service* service_;
  };
  using implementation_type = std::shared_ptr<strand_impl>;

  /// \brief 初始化函数
  /// \param context 上下文
  explicit gui_strand_executor_service(boost::asio::execution_context& context);
  /// \brief 关机
  void shutdown(){};
  /// \brief 创建
...
gui_strand_executor_service::gui_strand_executor_service(boost::asio::execution_context& context)
    : execution_context_service_base<gui_strand_executor_service>(context) {
}

template <typename Executor, typename Function>
void gui_strand_executor_service::execute(
    const gui_strand_executor_service::implementation_type& impl, Executor& ex,
    BOOST_ASIO_MOVE_ARG(Function) function,
    typename std::enable_if_t<
        boost::asio::can_query<Executor,
  ...
  //      std::allocator<void>());
  return gui_strand_executor_service::do_execute(
      impl, ex,
      BOOST_ASIO_MOVE_CAST(Function)(function),
...
template <typename Executor, typename Function, typename Allocator>
void gui_strand_executor_service::dispatch(
    const gui_strand_executor_service::implementation_type& impl, Executor& ex,
    BOOST_ASIO_MOVE_ARG(Function) function, const Allocator& a) {
...
gui_strand_executor_service::strand_impl::~strand_impl() {
  std::lock_guard l_g{service_->mutex_};
}

gui_strand_executor_service::implementation_type
gui_strand_executor_service::create_implementation() {
...
#pragma endregion
  ...
  execute(BOOST_ASIO_MOVE_ARG(Function) f) const {
    detail::gui_strand_executor_service::execute(
        impl_,
  ...
  void dispatch(BOOST_ASIO_MOVE_ARG(Function) f, const Allocator& a) const {
    detail::gui_strand_executor_service::dispatch(
        impl_,
  ...
  void post(BOOST_ASIO_MOVE_ARG(Function) f, const Allocator& a) const {
    detail::gui_strand_executor_service::post(impl_,
                                          executor_, BOOST_ASIO_MOVE_CAST(Function)(f), a);
  }

  template <typename Function, typename Allocator>
  void defer(BOOST_ASIO_MOVE_ARG(Function) f, const Allocator& a) const {
    detail::gui_strand_executor_service::defer(impl_,
                                           executor_, BOOST_ASIO_MOVE_CAST(Function)(f), a);
  }

  bool running_in_this_thread() const BOOST_ASIO_NOEXCEPT {
    return detail::gui_strand_executor_service::running_in_this_thread(impl_);
  }
  ...
  typedef detail::gui_strand_executor_service::implementation_type
      implementation_type;
  ...
  static implementation_type create_implementation(
      ...
      typename std::enable_if<
          ...
          std::int32_t>::type = 0) {
    return boost::asio::use_service<detail::gui_strand_executor_service>(
