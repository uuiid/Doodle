...
class kitsu_file_path {
 ...
 public:
  ...
  // 过滤已经存在的任务
  static std::vector<bool> filter_exist(pooled_connection& in_comm, const std::vector<kitsu_file_path>& in_task);
  static void insert(
      pooled_connection& in_comm, const std::vector<kitsu_file_path>& in_task,
      const std::map<entt::entity, boost::uuids::uuid>& in_map_id
  );
  static void update(pooled_connection& in_comm, const std::vector<kitsu_file_path>& in_task);
