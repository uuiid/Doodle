...
void project_helper::seed_to_sql(const entt::registry& in_registry, const std::vector<entt::entity>& in_entity) {
  for (auto&& l_entity : in_entity) {
    if (!in_registry.valid(l_entity)) continue;

    entt::const_handle l_handle{in_registry, l_entity};
    auto& l_p = l_handle.get<project>();
    database_t l_r{
        .id_               = boost::numeric_cast<std::int32_t>(entt::to_integral(l_handle.entity())),
        .name_             = l_p.p_name,
