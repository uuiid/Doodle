...
class attendance_block {
 ...
 public:
  ...
  // 过滤已经存在的任务
  static std::vector<bool> filter_exist(pooled_connection& in_comm, const std::vector<attendance_block>& in_task);
  static void insert(
      pooled_connection& in_comm, const std::vector<attendance_block>& in_task,
      const std::map<entt::entity, boost::uuids::uuid>& in_map_id
  );
  static void update(pooled_connection& in_comm, const std::vector<attendance_block>& in_task);
