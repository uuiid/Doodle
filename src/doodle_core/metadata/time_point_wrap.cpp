...
void to_json(nlohmann::json& j, const time_point_wrap& p) { j["time"] = p.p_i->zoned_time_.get_sys_time(); }
void from_json(const nlohmann::json& j, time_point_wrap& p) {
  p.p_i->zoned_time_ = j.at("time").get<time_point_wrap::time_point>();
}

time_point_wrap::time_point_wrap() : p_i(std::make_unique<impl>()) {
  p_i->zoned_time_ = date::make_zoned(date::current_zone(), time_point::clock::now());
}

time_point_wrap::time_point_wrap(time_point in_utc_timePoint) : time_point_wrap() { set_time(in_utc_timePoint); }

time_point_wrap::time_point_wrap(time_local_point in_local_time_point) : time_point_wrap() {
...
void time_point_wrap::set_time(const time_point_wrap::time_local_point& in) { p_i->zoned_time_ = in; }
void time_point_wrap::set_time(const time_point_wrap::time_point& in) { p_i->zoned_time_ = in; }

time_point_wrap time_point_wrap::now() { return time_point_wrap{time_point::clock::now()}; }
time_point_wrap::time_point time_point_wrap::get_sys_time() const { return p_i->zoned_time_.get_sys_time(); }
time_point_wrap::time_local_point time_point_wrap::get_local_time() const { return p_i->zoned_time_.get_local_time(); }

time_point_wrap::~time_point_wrap() = default;
time_point_wrap::time_point_wrap(const time_point_wrap& in_other) noexcept : p_i(std::make_unique<impl>()) {
  *p_i = *in_other.p_i;
}
time_point_wrap::time_point_wrap(time_point_wrap&& in_other) noexcept : p_i(std::move(in_other.p_i)) {}
