...
void shot::set_shot_ab(const std::string& ShotAb) noexcept {
  p_shot_enum = magic_enum::enum_cast<shot_ab_enum>(ShotAb).value_or(shot_ab_enum::None);
}
...
bool shot::operator>(const shot& rhs) const { return rhs < *this; }
bool shot::operator<=(const shot& rhs) const { return !(rhs < *this); }
bool shot::operator>=(const shot& rhs) const { return !(*this < rhs); }
