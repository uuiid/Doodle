...
void sql_com<doodle::redirection_path_info>::insert(conn_ptr& in_ptr, const std::vector<entt::entity>& in_id) {
  ...
  sql_com<std::string> l_path{};
  std::map<entt::entity, std::string> path_map{};
  ...
  // auto l_path_pre=l_conn.prepare(sqlpp::insert_into(l_path_table).set(
  //     l_path_table.entity_id=sqlpp::parameter(l_path_table.entity_id)
  ...
  for (auto& l_h : l_handles) {
    ...
    //   path_map.emplace(l_h.entity(),l_p);
    // }
...
void sql_com<doodle::redirection_path_info>::select(
  ...
  for (auto& row : l_conn(sqlpp::select(l_table.entity_id, l_table.redirection_file_name)
                              .from(l_table)
