...
void sql_com<doodle::redirection_path_info>::insert(conn_ptr &in_ptr, const entt::observer &in_observer){
    ...
    for(auto& l_h:l_handles){
        ...
        DOODLE_LOG_INFO("插入数据库id {} -> 实体 {} 组件 {} ", l_r, l_h.entity(), rttr::type::get<redirection_path_info>().get_name());
    }
}
...
void sql_com<doodle::redirection_path_info>::select(conn_ptr& in_ptr, const std::map<std::int64_t, entt::entity>& in_handle) {
  auto& l_conn = *in_ptr;
  sql::RedirectionPathInfo l_table{};
