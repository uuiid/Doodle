...
void sql_com<doodle::database>::insert(conn_ptr& in_ptr, const std::vector<std::int64_t>& in_id) {
  ...
  auto l_pre = l_conn.prepare(sqlpp::insert_into(l_tabl).set(l_tabl.uuidData = sqlpp::parameter(l_tabl.uuidData)));

  for (auto& l_h : l_handles) {
    auto& l_data          = l_h.get<database>();
    l_pre.params.uuidData = uuids::to_string(l_data.uuid());
...
void sql_com<doodle::database>::select(conn_ptr& in_ptr, std::map<std::int64_t, entt::entity>& in_handle) {
  ...
  for (auto& row : l_conn(sqlpp::select(l_tabl.id, l_tabl.uuidData).from(l_tabl).unconditionally())) {
