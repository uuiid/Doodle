...
void _get_ctx_sql_data_(const entt::registry& in_registry, std::map<std::uint32_t, std::string>& in_data) {
...
template <typename... Type_T>
void get_ctx_sql_data(const entt::registry& in_registry, std::map<std::uint32_t, std::string>& in_data) {
...
void select_ctx_template(entt::registry& in_reg, sqlpp::sqlite3::connection& in_conn) {
  std::map<std::uint32_t, std::function<void(entt::registry & in_reg, const std::string& in_str)>> l_fun{
      std::make_pair(entt::type_id<Type>().hash(), [&](entt::registry& in_reg, const std::string& in_str) {
        ...
        if (in_reg.ctx().template contains<Type>()) in_reg.ctx().template erase<Type>();

        in_reg.ctx().template emplace<Type>(std::move(l_json.get<Type>()));
      })...};
