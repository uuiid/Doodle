...
work_clock2::work_clock2() = default;

void work_clock2::cut_interval(const time_type& in_min, const time_type& in_max) {
  auto l_d = discrete_interval_time::closed(in_min, in_max);
  interval_set_time_ &= l_d;
  interval_map_time_ &= l_d;
}

work_clock2::duration_type work_clock2::operator()(const time_type& in_min, const time_type& in_max) const {
  auto l_d = discrete_interval_time::closed(in_min, in_max);
  auto l_l = interval_set_time_ & l_d;
  duration_type l_len{};
  for (const auto& l_i : l_l) {
...
work_clock2::time_type work_clock2::next_time(const time_type& in_begin, const duration_type& in_du) const {
  auto l_d = discrete_interval_time::right_open(in_begin, time_type::max());
  auto l_l = interval_set_time_ & l_d;
  duration_type l_len{};
  for (auto&& l_i : l_l) {
...
std::vector<std::pair<work_clock2::time_type, work_clock2::time_type>> work_clock2::get_work_du(
    const time_type& in_min, const time_type& in_max
) const {
  std::vector<std::pair<time_type, time_type>> l_r{};
  auto l_d = discrete_interval_time::closed(in_min, in_max);
...
std::string work_clock2::debug_print() const {
  return fmt::format("时间段 {}  时间信息 {}", interval_set_time_, interval_map_time_);
...
work_clock2::time_type work_clock2::next_point(const work_clock2::time_type& in_point) {
  auto l_d = discrete_interval_time::right_open(in_point, time_type::max());
  auto l_l = interval_set_time_ & l_d;
  for (auto&& l_i : l_l) {
    return boost::icl::lower(l_i);
