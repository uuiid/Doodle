...
create table if not exists entity (
    id integer auto_increment constraint entity_pk primary key,
    uuid_data text,
...
create index if not exists usertab_permission_group_index on usertab (permission_group);
create index if not exists usertab_user_name_index on usertab (user_name);
create trigger if not exists UpdataLastTime_
AFTER
UPDATE OF json_data ON com_entity begin
update entity
