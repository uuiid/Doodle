...
MetadataWidget::MetadataWidget(wxWindow* in_window, wxWindowID in_id)
  ...
  k_layout_1->Add(p_list_view_ctrl_, wxSizerFlags{7}.Expand().Border(wxDirection::wxALL, 0));
  ...
  p_tree_view_ctrl_->Bind(
      wxEVT_DATAVIEW_ITEM_CONTEXT_MENU, &MetadataWidget::treeContextMenu, this);
  ...
  p_tree_view_ctrl_->Bind(
      ...
      [this](wxDataViewEvent& in_event) {
        ...
        this->p_list_attribute_model->setRoot(k_m->shared_from_this());
      });

  MetadataSet::Get().sig_projectChange.connect(
      [this](doodle::Project* in_prj, int index) {
        this->p_assstsTree_model->setRoot(std::dynamic_pointer_cast<Project>(in_prj->shared_from_this()));
      });
...
void MetadataWidget::treeContextMenu(wxDataViewEvent& in_event) {
  wxMenu k_wx_menu{};
  ContextMenu k_context_menu{this, &k_wx_menu, p_assstsTree_model.get()};
...
void MetadataWidget::projectContextMenu(wxDataViewEvent& in_event) {
  wxMenu k_wx_menu{};
  ContextMenu k_context_menu{this, &k_wx_menu, p_project_model.get()};
  auto k_data = in_event.GetItem();
  if (k_data.IsOk()) {
    auto k_item = reinterpret_cast<Metadata*>(k_data.GetID());
    k_item->createMenu(&k_context_menu);
