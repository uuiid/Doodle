...
std::vector<action_ptr> menu_factory::operator()(MetadataPtr& in_metadata) {
  if (typeid(in_metadata) == typeid(Project*)) {
    create_prj_action(in_metadata);
  } else if (typeid(in_metadata) == typeid(AssetsFile*)) {
    create_file_action(in_metadata);
  } else {
    create_ass_action(in_metadata);
  }
  return p_action;
}
...
void menu_factory::create_menu(MetadataPtr& in_metadata) {
  nana::menu k_menu{};
  auto k_as = (*this)(in_metadata);
  for (auto& k_i : k_as) {
    k_menu.append(
        k_i->class_name(),
