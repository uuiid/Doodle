...
class functional_registration_manager {
 private:
  std::map<std::string, std::function<nlohmann::json(const entt::handle &, const nlohmann::json &)>> map_;
 ...
 public:
  ...
  std::function<nlohmann::json(const entt::handle &, const nlohmann::json &)> get_function(std::string in_name);
};
