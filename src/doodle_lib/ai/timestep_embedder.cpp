...
void timestep_embedder::init(
  ...
  DOODLE_CHICK(
      sequence_pos_encoder_->d_model() == latent_dim, "timestep_embedder: PE 维度 {} 不匹配 latent_dim {}",
      sequence_pos_encoder_->d_model(), latent_dim
  );
  ...
  DOODLE_CHICK(
      linear1_.in_features() == latent_dim && linear1_.out_features() == latent_dim,
      "timestep_embedder linear1 维度不匹配: 期望 [{},{}], 实际 [{},{}]", latent_dim, latent_dim,
      linear1_.in_features(), linear1_.out_features()
  );
  DOODLE_CHICK(
      linear2_.in_features() == latent_dim && linear2_.out_features() == latent_dim,
      "timestep_embedder linear2 维度不匹配: 期望 [{},{}], 实际 [{},{}]", latent_dim, latent_dim,
      linear2_.in_features(), linear2_.out_features()
  );
}
