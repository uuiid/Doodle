...
Eigen::MatrixXf classifier_free_guided_model::forward(
  ...
  if (actual_cfg_type == "regular") {
    ...
    DOODLE_CHICK(
        out_cond_uncond.rows() == total_frames * 2, "regular CFG forward 输出行数 {} 不匹配期望 {}",
  ...
  if (actual_cfg_type == "separated") {
    ...
    DOODLE_CHICK(
        out_cond_uncond.rows() == total_frames * 3, "separated CFG forward 输出行数 {} 不匹配期望 {}",
    ...
    return out_uncond.array() + w_text * (out_text.array() - out_uncond.array()) +
