...
kimodo_motion_rep::kimodo_motion_rep(
  ...
  // 加载统计信息
  if (!stats_path.empty() && FSys::exists(stats_path)) {
    load_stats(stats_path);
    SPDLOG_INFO("kimodo_motion_rep: 从 {} 加载统计信息", stats_path.string());
  }

  SPDLOG_INFO(
      "kimodo_motion_rep 初始化完成: nbjoints={}, motion_rep_dim={}, global_root_dim={}, body_dim={}, local_root_dim={}",
...
Eigen::MatrixXf kimodo_motion_rep::encode(
    const Eigen::MatrixXf& local_joint_rots,
  ...
  // ---- Step 5: 速度 ----
  Eigen::MatrixXf velocities = compute_vel_xyz(
      fk_res.posed_joints, fps_, batch_size, time_steps, nbjoints_, actual_lengths
  );
...
Eigen::MatrixXf kimodo_motion_rep::rotate(
  ...
  std::int64_t offset = 0;
  result.middleCols(offset, s1) = new_smooth_root; offset += s1;
  result.middleCols(offset, s2) = new_heading; offset += s2;
  result.middleCols(offset, s3) = new_joints; offset += s3;
  result.middleCols(offset, s4) = new_rot_6d; offset += s4;
  result.middleCols(offset, s5) = new_vel; offset += s5;
...
Eigen::MatrixXf kimodo_motion_rep::translate_2d(
    const Eigen::MatrixXf& features,
    const Eigen::MatrixXf& translation_2d,
    std::int64_t batch_size,
    std::int64_t time_steps
) const {
  const Eigen::Index total = features.rows();
...
kimodo_motion_rep::condition_result kimodo_motion_rep::create_conditions(
    ...
    const std::unordered_map<std::string, std::vector<Eigen::MatrixXf>>& data_dict,
    std::int64_t length,
    bool to_normalize
) const {
