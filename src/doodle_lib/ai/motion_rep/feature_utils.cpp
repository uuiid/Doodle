...
// ======================================================================
Eigen::MatrixXf compute_vel_angle(
    const Eigen::MatrixXf& root_rot_angles,
    float fps,
    const Eigen::VectorXi& lengths
) {
...
rotate_features::rotate_features(const Eigen::VectorXf& angle) {
  ...
  for (Eigen::Index i = 0; i < B; ++i) {
    ...
    corrective_mat_Y_T_(i, 0) = cos_a;  corrective_mat_Y_T_(i, 1) = 0.0f;  corrective_mat_Y_T_(i, 2) = -sin_a;
    corrective_mat_Y_T_(i, 3) = 0.0f;   corrective_mat_Y_T_(i, 4) = 1.0f;  corrective_mat_Y_T_(i, 5) = 0.0f;
    corrective_mat_Y_T_(i, 6) = sin_a;  corrective_mat_Y_T_(i, 7) = 0.0f;  corrective_mat_Y_T_(i, 8) = cos_a;
  }
}
...
Eigen::MatrixXf rotate_features::rotate_2d_positions(
    const Eigen::MatrixXf& positions_2d,
    std::int64_t batch_size,
    std::int64_t time_steps
) const {
  // positions_2d: [B*T, 2]
...
Eigen::MatrixXf rotate_features::rotate_6d_rotations(
    const Eigen::MatrixXf& rotations_6d,
    std::int64_t batch_size,
    std::int64_t time_steps,
    std::int64_t nbjoints
) const {
  ...
  for (Eigen::Index i = 0; i < total_joints; ++i) {
    ...
    rot_mats(i, 0) = r00; rot_mats(i, 1) = r01; rot_mats(i, 2) = r02;
    rot_mats(i, 3) = r10; rot_mats(i, 4) = r11; rot_mats(i, 5) = r12;
    rot_mats(i, 6) = r20; rot_mats(i, 7) = r21; rot_mats(i, 8) = r22;
  }
