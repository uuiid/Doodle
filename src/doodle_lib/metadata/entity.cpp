...
std::tuple<std::string, uuid> get_full_name(const entity& in_entity) {
  ...
  if (in_entity.entity_type_id_ == asset_type::get_shot_id()) {
    auto l_seq = g_ctx().get<sqlite_database>().get_by_uuid<entity>(in_entity.parent_id_);
    ...
    if (!in_entity.parent_id_.is_nil()) {
      auto l_ep = g_ctx().get<sqlite_database>().get_by_uuid<entity>(in_entity.parent_id_);
      l_name_   = fmt::format("{}/{}", l_ep.name_, in_entity.name_);
      l_ep_id   = l_ep.uuid_id_;
    }
  } else {
    auto l_ass_type = g_ctx().get<sqlite_database>().get_by_uuid<asset_type>(in_entity.entity_type_id_);
