...
bool opencv_read_player::load_frame(std::int32_t in_frame) {
  ...
  if (p_image.count(in_frame) == 1) {
    return true;
  }

  frame k_f{};
  k_f.p_height = boost::numeric_cast<std::int32_t>(p_video.get(cv::CAP_PROP_FRAME_HEIGHT));
  k_f.p_width  = boost::numeric_cast<std::int32_t>(p_video.get(cv::CAP_PROP_FRAME_WIDTH));
  ...
  p_image.insert({in_frame, std::move(k_f)});
