...
bool opencv_read_player::load_frame(std::int32_t in_frame) {
  ...
  if (p_data->p_image.count(in_frame) == 0) {
    frame_impl k_f{};
    ...
    D3D11_TEXTURE2D_DESC k_tex_desc{};
    k_tex_desc.Width            = k_f.frame.height;
    k_tex_desc.Height           = k_f.frame.width;
