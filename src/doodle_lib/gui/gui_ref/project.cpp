...
void project_edit::save_(const entt::handle& in) const {
  ...
  auto& l_context = g_reg()->ctx();
  if (l_context.contains<project>())
    l_context.erase<project>();
  l_context.emplace<project>(in.get<project>());
}
...
void base_config_edit::init_(const entt::handle& in) {
  ...
  p_i->season_count = l_config.season_count;
}
...
void base_config_edit::save_(const entt::handle& in) const {
  ...
  l_c.season_count = p_i->season_count;
}
...
void base_config_edit::render(const entt::handle& in) {
  ...
  if (ImGui::InputInt(*p_i->season_count.gui_name, &p_i->season_count.data))
    set_modify(true);
}
