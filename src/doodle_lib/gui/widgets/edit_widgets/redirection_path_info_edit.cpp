...
redirection_path_info_edit::redirection_path_info_edit() : ptr(std::make_unique<impl>()) {}
void redirection_path_info_edit::render(const entt::handle& in) {
  if (dear::InputTextMultiline(*ptr->search_path_.gui_name, &ptr->search_path_.data)) set_modify(true);

  if (dear::InputText(*ptr->file_name_.gui_name, &ptr->file_name_.data)) set_modify(true);
}
void redirection_path_info_edit::init_(const entt::handle& in) {
  if (in.any_of<redirection_path_info>()) {
    ...
    ptr->search_path_ = fmt::to_string(fmt::join(
        l_info.search_path_ |
            ranges::views::transform([](const FSys::path& in_path) -> std::string { return in_path.generic_string(); }),
        "\n"
    ));
  } else {
...
void redirection_path_info_edit::save_(const entt::handle& in) const {
  in.emplace_or_replace<redirection_path_info>(
      ptr->search_path_.data | ranges::views::split('\n') | ranges::views::cache1 |
