      ...
      list = gui_data().line_list |
             ranges::views::transform([](const xlsx_export_widgets_ns::xlsx_line &in_line) -> xlsx_line_gui {
               ...
               return xlsx_line_gui{
                   ...
                   in_line.episodes_,
                   in_line.shot_,
                   fmt::to_string(in_line.start_time_.get_local_time()),
                   fmt::to_string(in_line.end_time_.get_local_time()),
...
void xlsx_export_widgets::export_xlsx() {
  ...
  for (auto i = 1; i < 100; ++i) {
    ...
    g_windows_manage().create_windows_arg(
        windows_init_arg{}
            .create<show_message>(fmt::format("成功导出到路径{}", p_i->export_path.data))
            .set_title("显示消息")
            .set_render_type<dear::Popup>()
    );
    break;
...
bool xlsx_export_widgets::get_work_time() {
  ...
  if (p_i->list.empty()) {
    g_windows_manage().create_windows_arg(
        windows_init_arg{}.create<show_message>("筛选后为空").set_title("显示消息").set_render_type<dear::Popup>()
    );
    return false;
  }
