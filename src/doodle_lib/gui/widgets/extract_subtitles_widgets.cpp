...
class extract_subtitles_widgets::impl {
 public:
  impl() = default;
  gui_cache<std::vector<std::string>> file_list_{"文件列表"s, std::vector<std::string>{}};
...
bool extract_subtitles_widgets::render() {
  ...
  if (ImGui::Button(*p_i->export_)) {
    ranges::for_each(p_i->file_list_.data, [&](const FSys::path& in_string) {
      ...
      if (p_i->export_file_path_.data.empty()) l_out = in_string;
...
void extract_subtitles_widgets::write_subtitles(
    const FSys::path& in_source_file, const FSys::path& out_subtitles_file
) {
  boost::contract::check _l_chick = boost::contract::function().postcondition([&]() {
    FSys::is_regular_file(in_source_file) ? void() : throw_exception(doodle_error{"不存在文件 {}"s, in_source_file});
    (in_source_file.extension() == ".txt") ? void()
  ...
  for (std::string l_string{}; std::getline(l_in_f, l_string);) {
    if (std::regex_match(conv::utf_to_utf<wchar_t>(l_string), l_regex)) l_subtitles.push_back(l_string);
  }

  chrono::seconds l_seconds{0h + 0min + 0s};
  l_subtitles = l_subtitles | ranges::views::enumerate |
                ranges::views::transform([&](const std::pair<std::size_t, std::string>& in_pair) -> std::string {
...
void extract_subtitles_widgets::init() {
  p_i->scoped_connection_ =
      g_ctx().get<core_sig>().select_handles.connect([this](const std::vector<entt::handle>& in_list) {
