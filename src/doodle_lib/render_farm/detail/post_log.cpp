...
void post_log::operator()(boost::system::error_code ec, std::size_t bytes_transferred) const {
  ...
  switch (impl_->msg_type_) {
    case log_enum::log:
      impl_->msg_handle_.get<process_message>().message(std::move(l_str), process_message::level::info);
      break;
    case log_enum::err:
      impl_->msg_handle_.get<process_message>().message(std::move(l_str), process_message::level::warning);
      break;
  }
}
}
