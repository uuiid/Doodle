...
namespace {
constexpr std::size_t g_current_version = 9;
}
...
struct upgrade_2_t : sqlite_upgrade {
  ...
  void upgrade(sqlite_database& in_data) override {
    ...
    if (in_data.pragma().user_version() == 8) {
      in_data.sync_schema();
      ...
      auto l_entitys =
          select(in_data)
              ...
              .from<entity>()
              .where(c(&entity::entity_type_id_).in({asset_type::get_episode_id(), asset_type::get_ai_id()}))
