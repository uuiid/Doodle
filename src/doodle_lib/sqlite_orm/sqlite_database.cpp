...
void sqlite_storage::regs_all() {
  ...
  reg_table<seedance2::task>("seedance2_task_2")
      ...
      .add_foreign_key(&seedance2::task::subproject_id_, &seedance2::subproject::uuid_id_, foreign_key_action::set_null)
      .add_foreign_key(
  ...
  reg_table<seedance2::subproject>("seedance2_subproject")
      ...
      .add_column("preview_file", &seedance2::subproject::preview_file_)
  ...
  reg_table<seedance2::ai_episode>("seedance2_ai_episode")
      .add_column("id", &seedance2::ai_episode::id_, primary_key(), autoincrement())
      .add_column("uuid_id", &seedance2::ai_episode::uuid_id_, unique(), not_null())
...
boost::asio::awaitable<void> sqlite_database::backup(FSys::path in_path) {
  ...
  core_set::get_set().database_->create_session().backup_to(in_path);
...
person sqlite_database::get_person_for_email(const std::string& in_email) {
  using namespace orm;
  auto l_p = select(*this).columns(object<person>()).from<person>().where(c(&person::email_) == in_email)().to_single();
  return l_p;
}
