...
struct count_t {
  static constexpr std::size_t value = sizeof...(Column);
  std::tuple<std::decay_t<Column>...> columns_tuple_;
  explicit count_t(std::tuple<std::decay_t<Column>...> columns_tuple) : columns_tuple_(std::move(columns_tuple)) {}
};
// 是一个 count_t 的模板特化
template <typename... Column>
...
struct count_column_info_t : public base_column_info_t {
  std::vector<table_columns_t> column_infos_;
  template <typename... Column>
  explicit count_column_info_t(count_t<Column...>&& count) {
    column_infos_ = {table_columns_t(std::get<std::decay_t<Column>>(count.columns_tuple_))...};
  }
