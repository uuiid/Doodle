...
#define DOODLE_SELECT_VALUE_FIELD_DECL_(r, data, i, elem) \
  std::tuple_element_t<i, select_tuple_type> BOOST_PP_TUPLE_ELEM(2, 1, elem);
...
#define DOODLE_SELECT_VALUE(struct_name, ...)                                                                \
  struct struct_name {                                                                                       \
    ...
    BOOST_PP_SEQ_FOR_EACH_I(DOODLE_SELECT_VALUE_FIELD_DECL_, _, BOOST_PP_VARIADIC_TO_SEQ(__VA_ARGS__))      \
  };
