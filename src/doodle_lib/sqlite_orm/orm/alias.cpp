...
// }
std::string alias_info_t::to_sql(const session& s, const to_sql_ctx& ctx) const {
  if (table_name_.empty()) throw std::runtime_error("Table name is required for alias");
  return fmt::format("{} AS {}", s.get_table_name(table_type_index_), table_name_);
}
...
std::string alias_column_info_t::get_column_name(const session& s, const to_sql_ctx& ctx) const {
  auto l_ctx = ctx;
...
std::string rank_info_t::get_column_name(const session& s, const to_sql_ctx& ctx) const { return "rank"; }
...
std::string any_column_info_t::get_column_name(const session& s, const to_sql_ctx& ctx) const {
