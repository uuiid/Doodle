...
struct bind_value_t {
  ...
  std::function<std::string(const bind_value_t&, session& in_session, const to_sql_ctx& ctx)> to_string_fun_;
  ...
  std::string to_string(session& in_session, const to_sql_ctx& ctx) const;
};
