...
bind_value_t::bind_value_t(column_info_ptr column_info) {
  ...
  to_string_fun_ = [](const bind_value_t& self, const session& in_storage, const to_sql_ctx& ctx) {
...
std::string bind_value_t::to_string(const session& in_storage, const to_sql_ctx& ctx) const {
