...
inline auto make_storage_doodle(const std::string& in_path, sqlite_database_impl* impl) {
  ...
  return std::move(make_storage(
      ...
      make_table<seedance2::task>(
          ...
          make_column("task_id", &seedance2::task::task_id_), make_column("created_at", &seedance2::task::created_at_),
          make_column("ended_at", &seedance2::task::ended_at_),
          make_column("shot_uuid_id", &seedance2::task::shot_uuid_id_),
          make_column("archived", &seedance2::task::archived_),
          foreign_key(&seedance2::task::shot_uuid_id_).references(&task::uuid_id_).on_delete.set_null()
      ),
      ...
      make_trigger(
          "entity_fts_delete_trigger2",
          before()
              .delete_()
              .on<entity_asset_extend>()
              .begin(  //
                  update_all(
                      set(c(&entity_fts::bian_hao_) = "", c(&entity_fts::pin_yin_ming_cheng_) = ""),
                      where(c(&entity_fts::entity_id_) == old(&entity_asset_extend::entity_id_))
                  )
              )
              .end()
      ...
      make_table<playlist_shot>(
          ...
          make_column("order_index", &playlist_shot::order_index_),
      ...
      make_table<server_task_info>(
          ...
          make_column("task_id", &server_task_info::task_id_),            //
          ...
          foreign_key(&server_task_info::task_id_).references(&task::uuid_id_).on_delete.cascade()
      ),
      ...
      make_table<entity_asset_extend>(
          ...
          make_column("entity_id", &entity_asset_extend::entity_id_, not_null()),                      //
