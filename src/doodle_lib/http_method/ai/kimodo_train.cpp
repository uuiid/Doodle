...
DOODLE_HTTP_FUN_OVERRIDE_IMPLEMENT(ai_train_animation, post) {
  ...
  if (l_segment.skeleton_) {
    ...
    auto l_root_pos   = l_segment.skeleton_->neutral_joints_.row(l_root_index).head<3>();
    l_segment.skeleton_->neutral_joints_.rowwise() -= l_root_pos;
    l_segment.root_trajectory_ += l_root_pos;
    DOODLE_CHICK(
        (l_segment.skeleton_->neutral_joints_.row(l_root_index).head<3>() == Eigen::RowVector3f{0, 0, 0}),
        "根节点位置不为零"
    );
    for (auto& seg : l_segment.segments_)
