  ...
  for (auto& l_project : l_projects) {
...

    {
      ...
      for (auto&& [l_entity_id, l_jishu_size] : select(l_sql)
        ...
        if (l_entity_map.contains(l_entity_id)) l_project.episodes_[l_entity_map[l_entity_id]].count_ = l_jishu_size;
      }
    }
...
boost::asio::awaitable<boost::beast::http::message_generator> user_context::get(session_data_ptr in_handle) {
  ...
  }
  {
    ...
    for (auto&& l_person : l_persons) l_person_map[l_person.uuid_id_] = &l_person;
    ...
    for (auto&& [l_dep_id, l_person] :
      ...
      if (l_person_map.contains(l_person)) l_person_map[l_person]->departments_.push_back(l_dep_id);
    }
