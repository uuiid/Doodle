...
boost::asio::awaitable<boost::beast::http::message_generator> user_authenticated(session_data_ptr in_handle) {
  ...
  try {
    ...
    if (l_user_ptr->mobile_ != l_user["mobile"].get<std::string>() ||
        l_user_ptr->power_ != l_user["power"].get<power_enum>()) {
      l_user_ptr->mobile_ = l_user["mobile"].get<std::string>();
      l_user_ptr->power_  = l_user["power"].get<power_enum>();
      l_user_ptr->uuid_id_ = l_user_id;
      if (auto l_e = co_await g_ctx().get<sqlite_database>().install(l_user_ptr)) {
