...
void task_server::init() {
  for (auto&& [e, l_task] : g_reg()->view<doodle::server_task_info>().each()) {
    task_entity_map_[l_task.uuid_id_] = e;
    task_map_[l_task.uuid_id_]        = std::make_shared<doodle::server_task_info>(l_task);
  }
  clear_task();
}
...
boost::asio::awaitable<void> task_server::add_task(entt::entity in_entity) {
  ...
  task_entity_map_[l_task.uuid_id_] = in_entity;
  task_map_[l_task.uuid_id_]        = std::make_shared<doodle::server_task_info>(l_task);
}
...
boost::asio::awaitable<void> task_server::assign_task() {
  ...
  for (auto&& [id_, l_task] : task_map_) {
    ...
    for (auto& l_computer : l_computers) {
      if (l_computer->computer_data_ptr_->server_status_ != computer_status::free ||
          l_computer->computer_data_ptr_->client_status_ != computer_status::free)
        continue;
      ...
      if (auto l_c = l_computer->client.lock(); l_c) {
        logger_ptr_->log(
            log_loc(), level::info, "分配任务 {}_{} 给 {}({})", l_task->name_, id_, l_computer->computer_data_ptr_->name_,
            l_computer->computer_data_ptr_->ip_
        );
