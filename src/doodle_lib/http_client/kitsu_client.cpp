...
boost::asio::awaitable<std::string> kitsu_client::get_entity_name(uuid in_entity_id) const {
  boost::beast::http::request<boost::beast::http::empty_body> l_req{
      boost::beast::http::verb::get, fmt::format("/api/data/entities/{}", in_entity_id), 11
  };
  set_req_headers(l_req);
  ...
  if (l_res.result() != boost::beast::http::status::ok)
    throw_exception(doodle_error{"kitsu get entity error {} {}", l_res.result(), l_res.body().dump()});
  co_return l_res.body().value("name", "");
}
...
boost::asio::awaitable<void> kitsu_client::upload_shot_animation_auto_light(
  ...
  for (auto&& l_path : in_file_path) {
    SPDLOG_LOGGER_INFO(logger_, "上传文件 {}", l_path.local_path_);
    co_await upload_asset_file(
        fmt::format("/api/doodle/data/shots/{}/file/auto-light", in_shot_task_id), l_path.local_path_,
        base64_encode(l_path.field_name_)
    );
  }
  co_return;
...
boost::asio::awaitable<nlohmann::json> kitsu_client::get_ue_assembly(uuid in_project_id, uuid in_shot_task_id) const {
  ...
  boost::beast::http::response<http::basic_json_body> l_res{};
  co_await http_client_ptr_->read_and_write(l_req, l_res, boost::asio::use_awaitable);
  if (l_res.result() != boost::beast::http::status::created && l_res.result() != boost::beast::http::status::ok)
...
void kitsu_client::put_job_log_sync(const uuid& in_job_id, const std::string& in_log) const {
  boost::beast::http::request<boost::beast::http::string_body> l_req{
      boost::beast::http::verb::put, fmt::format("/api/actions/jobs/{}/log", in_job_id), 11
  };
  set_req_headers(l_req, "text/plain");
  l_req.body() = in_log;
  boost::beast::http::response<boost::beast::http::string_body> l_res{};
