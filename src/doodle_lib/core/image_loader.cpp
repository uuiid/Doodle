...
std::tuple<cv::Mat, std::shared_ptr<void>> image_loader::load_mat(const FSys::path& in_path) {
  ...
  if (exists(l_local_path) &&
    ...
    chick_true<doodle_error>(!k_image.empty(), "open cv not read image");
...
bool image_loader::load(const entt::handle& in_handle) {
  chick_true<doodle_error>(in_handle.any_of<image_icon>(), "缺失图标组件");
  chick_true<doodle_error>(in_handle.registry()->ctx().contains<project>(), "缺失项目上下文");
...
bool image_loader::save(const entt::handle& in_handle,
  ...
  auto k_reg = g_reg();
  chick_true<doodle_error>(k_reg->ctx().contains<project>(), "缺失项目上下文");
...
std::shared_ptr<void> image_loader::cv_to_d3d(const cv::Mat& in_mat, bool convert_toRGBA) const {
  ...
  chick_true<doodle_error>(k_r == 0, "windows com 异常 {}", k_r);
...
bool image_loader::save(const entt::handle& in_handle, const FSys::path& in_path) {
  auto k_reg = g_reg();
  chick_true<doodle_error>(k_reg->ctx().contains<project>(), "缺失项目上下文");
  chick_true<doodle_error>(exists(in_path), "文件不存在");
