...
std::vector<scan_category_data_ptr> prop_scan_category_t::scan(const std::shared_ptr<project_minimal> &in_root) const {
  const FSys::path l_prop_path_root = FSys::path{in_root->path_} / base_path_;
