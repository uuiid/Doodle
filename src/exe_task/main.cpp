...
class main_facet : public doodle::main_facet {
 ...
 protected:
  void load_windows() override {
    using namespace doodle;
    g_reg()->ctx().get<gui::layout_tick>() = std::make_shared<layout>();
    entt::handle{*g_reg(), g_reg()->create()}.emplace<gui::gui_tick>() = std::make_shared<menu_bar>();
    entt::handle{*g_reg(), g_reg()->create()}.emplace<gui::gui_tick>() = std::make_shared<gui::main_status_bar>();
  }
};
