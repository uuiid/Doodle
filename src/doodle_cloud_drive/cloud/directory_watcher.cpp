...
void directory_watcher::read_changes_async() {
  ...
  BOOL const l_ok = ::ReadDirectoryChangesW(
      dir_.get(), notify_.get(), sizeof(FILE_NOTIFY_INFORMATION) * 100, TRUE,
...
void directory_watcher::cancel() { ::CancelIoEx(dir_.get(), overlapped_ptr_->get()); }
...
void directory_watcher::init() {
  ...
  boost::asio::use_service<boost::asio::detail::io_context_impl>(
      ...
      .register_handle(dir_.get(), l_code);
  if (l_code) {
