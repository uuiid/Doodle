...
MDagPath qcloth_shape::cloth_mesh() const {
  ...
  MObject l_mesh{};
  /// \brief 获得组件点上下文
  display_info(fmt::format("使用q布料节点 {}", get_node_full_name(obj)));
...
void qcloth_shape::add_field(const reference_file& in_handle) const {
  ...
  if (l_f) {
    ...
    display_info("开始设置解算布料 {} 关联的风场", l_mesh);
    MStatus l_status{};
...
void qcloth_shape::rest() const {
  ...
  display_info("开始寻找布料对应的初识姿势 {} -> {}", get_node_name(l_path), l_name);
  ...
  if (!l_status) {
    display_warning("布料对应的初识姿势 {} 设置出错", l_name);
  }
}
