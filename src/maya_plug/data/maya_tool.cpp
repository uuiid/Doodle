...
MPlug get_plug(const MObject& in_node, const std::string& in_name) {
  in_node.isNull() ? throw_exception(doodle_error{"传入空节点寻找属性 {}", in_name}) : void();
  MStatus k_s{};
  MPlug l_plug{};
  ...
  try {
    ...
    display_info("没有在这个节点中找到属性 {}", in_name);
  }
  if (!l_plug.isNull()) {
    return l_plug;
  }
  ...
  if (in_node.hasFn(MFn::kDagNode)) {
    ...
    try {
      ...
      display_info("节点下方没有 shape 形状节点, 不需要寻找形状节点");
    }
  }
...
MObject get_shading_engine(const MDagPath& in_node) {
  ...
  for (MItDependencyGraph i{
    ...
    //    display_info(fmt::format("找到节点 {}", d_str{k_node.name()}.str()));
    break;
  }
...
MDagPath marge_mesh(const MSelectionList& in_marge_obj, const std::string& in_marge_name) {
  ...
  if (k_select.length(&k_s) > 1) {
    ...
    display_info("开始合并网格体 {}", fmt::join(l_names, " "));
