...
//
MHWRender::DrawAPI hud_render_override::supportedDrawAPIs() const { return MHWRender::kAllDevices; }
bool hud_render_override::startOperationIterator() {
  mCurrentOperation = 0;
  return true;
}

MHWRender::MRenderOperation *hud_render_override::renderOperation() {
...
MStatus hud_render_override::setup(const MString &destination) {
  ...
  if (!mOperations[0] || !mOperations[1]) {
    return MStatus::kFailure;
  }

  return MStatus::kSuccess;
}
hud_render::hud_render(const MString &in_name) : MUserRenderOperation(in_name) { mName = in_name; }

bool hud_render::hasUIDrawables() const { return true; }
void hud_render::addUIDrawables(
    MHWRender::MUIDrawManager &drawManager2D, const MHWRender::MFrameContext &frameContext
) {
  // Start draw UI
  ...
  drawManager2D.text(
      MPoint(w * 0.5f, h * 0.91f), MString("Renderer Override Options Tester"), MHWRender::MUIDrawManager::kCenter
  );
