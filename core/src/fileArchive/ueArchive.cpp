...
void ueArchive::insertDB() {
  dpathList list = p_server_path;
  list.push_back(p_server_path.front().parent_path() / DOODLE_CONTENT);
...
void ueArchive::_generateFilePath() {
  if (!p_soureFile.empty()) {
    p_server_path.push_back(p_info_->generatePath(
        "Scenefiles", boost::filesystem::extension(p_soureFile.front())));
  } else if (!p_info_->getFileList().empty())
    for (const auto &item : p_info_->getFileList()) p_server_path.push_back(item);
  //
