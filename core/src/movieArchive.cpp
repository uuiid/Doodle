...
void movieArchive::insertDB() {
  ...
  shotInfo->setFileType(shotInfo->findFileType("movie"));
...
bool movieArchive::makeMovie(const QString &imageFolder) {
  ...
  auto ffmpeg = std::make_unique<ffmpegWrap>(ffmpeg_exe.absolutePath() + "/ffmpeg.exe");
  std::vector<QString> list;
  for (auto &&item :image_list.toStdList()) list.push_back(item.absoluteFilePath());
  ffmpeg->imageToVideo(list, p_cacheFilePath.front(), QFileInfo(p_cacheFilePath.front()).baseName());
  return true;
}
bool movieArchive::update(const stringList &filelist) {
  ...
  if (filelist.size() == 1)
    if (QFileInfo(filelist.front()).isDir())
      makeMovie(filelist.front());

  return fileArchive::update(filelist);
}
