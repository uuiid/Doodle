...
> 状态：**方案设计 v3.2**（执行形态已澄清：无 UE 实时交互，单一自动模式 + 每轮人工判断）
...
如果每轮只给美术一个结果、只问「行不行」，人工判断只是 yes/no，收敛会慢到不可用。做法是每轮沿**语义可解释的轴**（主光方向 / 光比 / 色温 / 硬软 / 环境光水平 / 调色强度）生成 4–9 个候选，让美术**挑**而不是**调**。美术的选择直接翻译成下一轮的搜索方向——这才是把人工判断变成梯度的关键。
...
| 7 | **后处理/调色必须可改**（判断五） | 从"允许"升级为"必需"；执行器需有 grade 写入原语；Spec 要区分"灯光负责"与"调色负责" |
...
多张参考是常态，多视角联合约束是**可用的最强信号**：一套灯光必须同时解释所有视角。

- 未知量：光照表示 + 每张参考的相机位姿 + **每张图各自的调色变换（nuisance 参数）**
- 把 AI/PS 的**全局调色**吸收进 per-image nuisance，光照成为多图共享的解释
- 局部改图作为外点 → 鲁棒损失
- 位姿来源三级：render-and-compare → **人工对位（每张 1–2 分钟）** → 放弃

**建议**：Tier A 先落地；Tier B 只在 Tier A 的目标区间匹配明显不够时才上。
...
每轮固定若干**语义可解释的轴**，每轴 2–3 档，组合出 4–9 个候选：
...
第一批预设直接来自项目已有资产：`ADoodleAssetsPreview` 的 `LowContrast / MidContrast / HighContrast` 三档灯光环境。
...
### 7.3 一次启动，渲染 N 个候选（性能关键）

UE 进程启动成本高（数十秒到数分钟）。所以：

- **一个任务内含 N 个候选**，一次启动 UE 循环处理
...
| **M1 单轮管线** | 提交（提示词+多图）→ Spec → 候选生成 → 派发 → 渲染 → 前端展示 → 人工判断 | 完整跑通一轮：美术在浏览器上看到 N 个候选并能给出判断 |
...
> 与 v3.1 相比：原 M1「交互闭环（Slate 面板）」取消，替换为「单轮管线」；前端工作提前到 M1；M2 新增跨会话恢复。
...
| **人工判断往返慢、轮次少** | 每轮给 4–9 个沿轴变化的候选（一次往返换更多信息）；一次 UE 启动渲染全部候选；warm start |
...
| 反馈输入 | 选一个继续 + 自然语言反馈（"再冷一点"） | 新增；复用 AIScript 的对话式交互形态 |
...
1. **人工闸门 2 的粒度**：只做「候选级挑选」（本方案默认），还是需要「逐 op 接受/拒绝」？
   —— 后者需要为每个 op 渲染一张预览，成本是候选数的数倍，建议不做。
2. **每轮候选数量**：4 个、6 个还是 9 个？—— 直接影响每轮渲染时间与美术的浏览负担。
3. **反馈是否允许纯自然语言**（由 LLM 结构化），还是要求美术在轴上调滑块？
   —— 纯自然语言更顺手但可能结构化出错；滑块更精确但更繁琐。建议两者都支持，滑块作为默认。
4. **参考图的 AI/PS 调整以什么为主？** 全局调色为主，还是局部改图为主？
   —— 全局为主则 Tier B（M7）可行性大幅提升；局部为主则 Tier A 是唯一可靠路径。
5. **后处理/调色是否确认允许工具修改？**（决策 7 已按"允许"设计）
6. **每张参考是否值得让美术人工对位一次相机？**（每张 1–2 分钟）—— 决定 Tier B 是否值得做。
7. **性能预算**：单镜头允许的动态阴影灯上限、单轮（N 个候选）可接受的渲染时间。
8. **会话挂起的超时**：美术多久不判断算挂起？
9. **是否允许改动几何/材质**：当前范围限定在灯光 + 环境 + 后处理/调色。
10. **预设库首批规模**：是否安排美术整理一批（三点布光 / 伦勃朗 / 剪影 / 夜景霓虹…）。
